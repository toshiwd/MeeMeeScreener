# Mee Mee UI/UX 改善方針（桜井政博流UX × デザインシステム） v1.0

最終更新: 2026-01-06  
対象: Mee Mee（現行: Streamlit + カスタムHTML（仮想グリッド）前提）  
目的: 600銘柄超を「流し見で発見」し、気になった銘柄だけ深掘りできる“発見装置”にする

この文書は UI の具体設計だけを扱う。MeeMee 全体の判断軸、`売-買` 表記、通常画面と検証画面の分離、AI 出力原則は `docs/MEEMEE_PRINCIPLES.md` に集約する。
