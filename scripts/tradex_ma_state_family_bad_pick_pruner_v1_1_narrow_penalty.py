...
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\ma_state_family_bad_pick_pruner_v1_1_narrow_penalty")
...
SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_narrow_penalty"
COMPARE_SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_compare_v1"
DECISION_SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_decision_v1"
MANIFEST_SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_manifest_v1"
MONTHLY_SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_monthly_comparison_v1"
REGIME_SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_regime_comparison_v1"
PENALTY_COVERAGE_SCHEMA_VERSION = "tradex_ma_state_family_bad_pick_pruner_v1_1_penalty_coverage_v1"

TOP_K_VALUES = (5, 10, 20)
MAX_TOTAL_PENALTY = -0.22

NARROW_PENALTY_PROFILE = {
    "strict_bad_pick_family": -0.20,
    "relaxed_bad_pick_family": -0.08,
    "bad_pick_watch_family": 0.0,
    "regime_bad_pick_family": -0.08,
    "mae_risk_family": -0.02,
    "endpoint_bad_pick_family": 0.0,
}
...
def _progress_log(message: str) -> None:
    print(f"[ma_state_family_bad_pick_pruner_v1_1_narrow_penalty] {message}", file=sys.stderr, flush=True)
...
def _classify_penalty(row: pd.Series, penalty_profile: dict[str, float]) -> tuple[float, list[str], bool]:
    penalty = 0.0
    reasons: list[str] = []

    if bool(row.get("strict_bad_pick_family")):
        penalty += penalty_profile["strict_bad_pick_family"]
        reasons.append("strict_bad_pick_family")
    if bool(row.get("relaxed_bad_pick_family")):
        penalty += penalty_profile["relaxed_bad_pick_family"]
        reasons.append("relaxed_bad_pick_family")
    if bool(row.get("bad_pick_watch_family")) and penalty_profile["bad_pick_watch_family"] != 0.0:
        penalty += penalty_profile["bad_pick_watch_family"]
        reasons.append("bad_pick_watch_family")

    regime_match = bool(row.get("regime_bad_pick_family")) and str(row.get("family_regime_context") or "") == str(row.get("dominant_regime_context") or "")
    if regime_match:
        penalty += penalty_profile["regime_bad_pick_family"]
        reasons.append("regime_bad_pick_family")

    if bool(row.get("mae_risk_family")) and penalty_profile["mae_risk_family"] != 0.0:
        penalty += penalty_profile["mae_risk_family"]
        reasons.append("mae_risk_family")
    if bool(row.get("endpoint_bad_pick_family")) and penalty_profile["endpoint_bad_pick_family"] != 0.0:
        penalty += penalty_profile["endpoint_bad_pick_family"]
        reasons.append("endpoint_bad_pick_family")

    if penalty < MAX_TOTAL_PENALTY:
        penalty = MAX_TOTAL_PENALTY
        if "max_total_penalty_cap" not in reasons:
            reasons.append("max_total_penalty_cap")
    return float(penalty), reasons, regime_match


def _apply_pruner_scores(frame: pd.DataFrame, *, penalty_profile: dict[str, float]) -> pd.DataFrame:
    out = frame.copy()
    penalties = out.apply(lambda row: _classify_penalty(row, penalty_profile), axis=1, result_type="expand")
...
def _aggregate_selected_rows(frame: pd.DataFrame, *, selected_col: str, bottom15_threshold: float) -> dict[str, Any]:
    ...
    bad_pick_family = (
        selected["strict_bad_pick_family"].fillna(False).astype(bool)
        | selected["relaxed_bad_pick_family"].fillna(False).astype(bool)
        | selected["bad_pick_watch_family"].fillna(False).astype(bool)
        | selected["regime_bad_pick_family"].fillna(False).astype(bool)
        | selected["mae_risk_family"].fillna(False).astype(bool)
        | selected["endpoint_bad_pick_family"].fillna(False).astype(bool)
    )
    return {
...
def _build_compare_payload(
    *,
    ...
    topk_summary = {
        ...
        "selection_divergence_reason": "bad_pick_family_pruner_v1_vs_champion",
    ...
    return {
        ...
        "penalty_formula": {
            ...
            "relaxed_bad_pick_family": -0.08,
            "bad_pick_watch_family": 0.0,
            "regime_bad_pick_family_if_regime_matches": -0.08,
            "mae_risk_family": -0.02,
            "endpoint_bad_pick_family": 0.0,
            "max_total_penalty": MAX_TOTAL_PENALTY,
            "note": "penalties are additive per matched label and capped at the max total penalty",
        },
        "penalty_coverage": {},
...
def _build_penalty_coverage_summary(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    shadow_counts = {
        "strict_bad_pick_family": int(frame["strict_bad_pick_family"].fillna(False).astype(bool).sum()),
        "relaxed_bad_pick_family": int(frame["relaxed_bad_pick_family"].fillna(False).astype(bool).sum()),
        "bad_pick_watch_family": int(frame["bad_pick_watch_family"].fillna(False).astype(bool).sum()),
        "regime_bad_pick_family": int(frame["regime_bad_pick_family"].fillna(False).astype(bool).sum()),
        "mae_risk_family": int(frame["mae_risk_family"].fillna(False).astype(bool).sum()),
        "endpoint_bad_pick_family": int(frame["endpoint_bad_pick_family"].fillna(False).astype(bool).sum()),
    }
    return {
...
def _build_v1_vs_v1_1_delta_payload(
    *,
    ...
    return {
        "schema_version": "tradex_ma_state_family_bad_pick_pruner_v1_1_delta_v1",
        "generated_at": _utc_now(),
...
def run_ma_state_family_bad_pick_pruner_v1_1_narrow_penalty(
    *,
    ...
    _progress_log(
        f"start source_family={source_family_session_path} v1_session={v1_session_path} candidate_input={candidate_input_path} out_root={output_root_path} session={session_id}"
    )
    ...
    enriched = _apply_pruner_scores(enriched, penalty_profile=NARROW_PENALTY_PROFILE)
    ...
    diff_rows["bad_pick_family_contaminant"] = (
        diff_rows["strict_bad_pick_family"].fillna(False).astype(bool)
        | diff_rows["relaxed_bad_pick_family"].fillna(False).astype(bool)
        | diff_rows["bad_pick_watch_family"].fillna(False).astype(bool)
        | diff_rows["regime_bad_pick_family"].fillna(False).astype(bool)
        | diff_rows["mae_risk_family"].fillna(False).astype(bool)
        | diff_rows["endpoint_bad_pick_family"].fillna(False).astype(bool)
    )
    ...
    output_files = {
        ...
        "ma_state_family_bad_pick_pruner_v1_1_compare_json": session_tmp / "ma_state_family_bad_pick_pruner_v1_1_compare.json",
        "ma_state_family_bad_pick_pruner_v1_1_decision_json": session_tmp / "ma_state_family_bad_pick_pruner_v1_1_decision.json",
        "v1_vs_v1_1_delta_json": session_tmp / "v1_vs_v1_1_delta.json",
    ...
    v1_delta_payload = _build_v1_vs_v1_1_delta_payload(
    ...
    _write_json(output_files["ma_state_family_bad_pick_pruner_v1_1_compare_json"], compare_payload)
    _write_json(output_files["ma_state_family_bad_pick_pruner_v1_1_decision_json"], decision_payload)
    _write_json(output_files["v1_vs_v1_1_delta_json"], v1_delta_payload)
    ...
    return {
        ...
        "v1_vs_v1_1_delta": v1_delta_payload,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run TRADEX bad-pick pruner against the refined family map.")
