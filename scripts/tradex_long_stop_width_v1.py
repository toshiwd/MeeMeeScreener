...
STOPS=[1.0,1.5,2.0,2.5,3.0];TP=1.5
def run(r,sl):
 e=float(r.p1_o);stop=e*(1-sl/100);target=e*(1+TP/100)
 for n in range(1,6):
...
def main():
 ...
 for sl in STOPS:d[f'sl{sl}']=d.apply(lambda r:run(r,sl),axis=1)
 ...
 for sl in STOPS:
  parts=[]
  for f in [dev,val]:
   z=select_daily(f,'安定上昇',5);z['realized_ret']=z[f'sl{sl}'];parts.append(metrics(z))
  rows.append({'stop_loss_pct':sl,'discovery':parts[0],'validation':parts[1]})
 ...
 if chosen:sel['realized_ret']=sel[f"sl{chosen['stop_loss_pct']}"]
