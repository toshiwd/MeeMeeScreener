...
FROZEN_FAILURE_SESSION_ID = "entry-short-broad-down-monthlybreak-20260419-142728"
...
LADDER_VARIANTS = [
    trend.TrendVariant(
        name="broad_down_closepos_0p25_v1",
        target="bad-pick removal",
        weekly_breakout_down_min=0.66,
        monthly_breakout_down_min=0.30,
        monthly_range_prob_max=None,
    ...
    trend.TrendVariant(
        name="broad_down_closepos_0p30_v1",
        target="bad-pick removal",
        weekly_breakout_down_min=0.66,
        monthly_breakout_down_min=0.30,
        monthly_range_prob_max=None,
...
def _build_reentry_audit(
    *,
    ...
    return {
        "schema_version": "tradex_entry_precision_short_broad_down_closepos_reentry_audit_v1",
        "session_id": f"entry-short-broad-down-closepos-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}",
...
def _monthlybreak_compare(
    *,
    ...
    return {
        "schema_version": "tradex_entry_precision_short_broad_down_closepos_ablation_compare_v1",
        "session_id": f"entry-short-broad-down-closepos-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}",
        "generated_at": datetime.now(timezone.utc).isoformat(),
...
def _decision(compare: dict[str, Any], reentry: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    reasons = [
        ...
        f"profitable_reentered_0p25={reentry['reentry_summary']['broad_down_closepos_0p25_v1']['profitable_removed_reentered']}",
        f"profitable_reentered_0p30={reentry['reentry_summary']['broad_down_closepos_0p30_v1']['profitable_removed_reentered']}",
    ]
    ...
    return "drop", reasons + ["close_pos_not_the_remaining_shared_blocker"]
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    contract_payload = {
        "schema_version": "tradex_entry_precision_short_broad_down_closepos_contract_v1",
        ...
        "closepos_variants": [_variant_contract(v) for v in LADDER_VARIANTS],
    ...
    decision_payload = {
        "schema_version": "tradex_entry_precision_short_broad_down_closepos_fix_decision_v1",
        "session_id": session_id,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "baseline_id": BASELINE_ID,
        "challenger_id": CHALLENGER_ID,
        ...
        "confirmed": [
            ...
            "close_pos_only_ladder",
            "json_is_authoritative",
        ],
        "provisional": [
            "close_pos_is_partial_not_full_blocker",
            "broad_down_sample_is_still_small",
            "require_midrange_off_remains_for_4684",
        ],
        "remaining_risks": [
    ...
    report_payload = {
        "current_state": {
            "confirmed": [
                "monthly_breakout_down_min_was_partially_repaired",
                "monthly_alignment_failed_is_direct_gate",
                "long_logic_remained_frozen",
            ],
            "provisional": [
                "close_pos_may_only_be_a_partial_blocker",
                "require_midrange_off_may_be_the_remaining_blocker_for_4684",
                "regime_labels_are_proxy_derived",
            ],
        },
        "problem": "The task is to validate whether the remaining missing broad-down short is still blocked by close_pos_max after the monthly breakout repair.",
        "change_policy": "TRADEX research only, close_pos_max ladder only, keep long logic frozen, keep MeeMee untouched, keep require_midrange_off frozen, and do not change monthly gates again.",
        "frozen_failure_session_id": FROZEN_FAILURE_SESSION_ID,
        "compare": compare,
        ...
        "next_one_thing": "If this lane continues, the next single axis would be require_midrange_off only if close_pos proves insufficient.",
    }

    _write_json(out_dir / "entry_precision_short_broad_down_closepos_contract.json", contract_payload)
    _write_json(out_dir / "entry_precision_short_broad_down_closepos_ablation_compare.json", compare)
    _write_json(out_dir / "entry_precision_short_broad_down_closepos_reentry_audit.json", reentry_payload)
    _write_json(out_dir / "entry_precision_short_broad_down_closepos_quality_compare.json", compare)
    _write_json(out_dir / "entry_precision_short_broad_down_closepos_fix_decision.json", decision_payload)
    _write_report_md(out_dir / "entry_precision_short_broad_down_closepos_fix_report.md", report_payload)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Close-position threshold ladder for the frozen broad-down reference.")
