...
AXIS_ID = "forward_research_watch_validation_protocol_v1"
DEFAULT_FORWARD_SURFACE_ROOT = Path(
    r"G:\Tradex\forward_current_candidate_surface_v1\20260525T141438Z-forward-current-candidate-surface-v1"
)
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\forward_research_watch_validation_protocol_v1")
REQUIRED_ARTIFACTS = (
    "forward_watch_protocol_summary.json",
    "forward_watch_rows.csv",
    "forward_watch_contract.json",
...
def load_forward_surface(surface_root: Path) -> tuple[pd.DataFrame, dict[str, Any], dict[str, Any]]:
    rows_path = surface_root / "forward_current_candidate_surface_rows.parquet"
...
def build_watch_rows(surface: pd.DataFrame, top_n: int = WATCH_TOP_N) -> pd.DataFrame:
    required = {
        "as_of_date",
        "code",
        "forward_research_watch_score",
        "forward_research_watch_rank",
        "forward_research_watch_bucket",
        "forward_surface_live_feature_available_flag",
    }
    missing = sorted(required - set(surface.columns))
    if missing:
        raise ValueError(f"missing required forward surface columns: {missing}")
    watch = surface.sort_values(["forward_research_watch_rank", "code"]).head(top_n).copy()
    watch["forward_watch_protocol_rank"] = range(1, len(watch) + 1)
    watch["forward_watch_status"] = "research_watch_pending_outcome"
...
def build_validation_schedule(watch_rows: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "validation_windows": [
            {
                "window_id": "ret5_forward_evaluation",
                "outcome_column": "ret5",
                "required_future_confirmed_sessions": 5,
                "purpose": "diagnostic_only_short_horizon_check",
            },
            {
                "window_id": "ret20_forward_evaluation",
                "outcome_column": "ret20",
                "required_future_confirmed_sessions": 20,
                "purpose": "diagnostic_only_starter_entry_check",
            },
        ],
...
def future_outcome_join_contract() -> dict[str, Any]:
    return {
        ...
        "outcome_usage": "future_evaluation_only_after_watch_rows_are_frozen",
...
def no_lookahead_audit(watch_rows: pd.DataFrame, source_decision: dict[str, Any]) -> dict[str, Any]:
    ...
    present_forbidden = sorted(forbidden & set(watch_rows.columns))
    ...
    passed = not present_forbidden and source_safe
    return {
        ...
        "offline_outcome_columns_present_in_watch_rows": present_forbidden,
...
def source_coverage(surface: pd.DataFrame, watch_rows: pd.DataFrame, source_root: Path, source_coverage_payload: dict[str, Any]) -> dict[str, Any]:
    return {
        "axis_id": AXIS_ID,
        "source_surface_root": str(source_root),
        "source_row_count": int(len(surface)),
        "source_date_count": int(surface["as_of_date"].nunique()) if "as_of_date" in surface else 0,
        "source_code_count": int(surface["code"].nunique()) if "code" in surface else 0,
        ...
        "source_latest_feature_frame_date": source_coverage_payload.get("latest_feature_frame_date"),
        "research_fallback_used": False,
    }
...
def decide(watch_rows: pd.DataFrame, audit: dict[str, Any], source_decision: dict[str, Any]) -> tuple[str, str, list[str]]:
    if not audit["no_lookahead_pass"]:
        return "blocked_no_lookahead_violation", "BLOCKED", ["watch_rows_or_source_contract_failed_no_lookahead"]
    if source_decision.get("research_decision") != "forward_current_surface_ready_for_research_watch_pretest":
        return "blocked_missing_forward_surface_contract", "BLOCKED", ["source_surface_not_ready_for_research_watch_pretest"]
    if watch_rows.empty:
        return "blocked_missing_forward_watch_candidates", "BLOCKED", ["no_watch_rows_selected"]
    return "forward_watch_protocol_ready_for_future_outcome_validation", "HOLD_UNDERPOWERED", [
        "forward_watch_rows_frozen_for_future_ret5_ret20_evaluation",
        "not_buyable_until_future_outcomes_validate_selector",
    ]


def run(surface_root: Path = DEFAULT_FORWARD_SURFACE_ROOT, output_root: Path = DEFAULT_OUTPUT_ROOT, top_n: int = WATCH_TOP_N) -> Path:
    surface, source_decision, source_cov = load_forward_surface(surface_root)
    ...
    out = output_root / f"{_now_tag()}-forward-research-watch-validation-protocol-v1"
    ...
    keep_cols = [
        ...
        "forward_research_watch_rank",
        "forward_research_watch_bucket",
        "forward_watch_protocol_rank",
        "forward_watch_status",
        "buy_recommendation",
        "validated_buy",
        "active_gate_created",
    ...
    watch_rows[keep_cols].to_csv(out / "forward_watch_rows.csv", index=False)
    ...
    summary = {
        ...
        "watch_date_count": int(watch_rows["as_of_date"].nunique()) if not watch_rows.empty else 0,
        "watch_code_count": int(watch_rows["code"].nunique()) if not watch_rows.empty else 0,
        "top_watch_codes": watch_rows.sort_values("forward_watch_protocol_rank")["code"].astype(str).head(10).tolist(),
        "research_watch_only": True,
        "buyable_selection_ready": False,
        "validated_buy_count": 0,
    }
    _write_json(out / "forward_watch_protocol_summary.json", summary)
    _write_json(
        out / "forward_watch_contract.json",
        {
            "axis_id": AXIS_ID,
            "watch_selection_rule": "freeze_top_n_by_forward_research_watch_rank_from_authoritative_forward_current_surface",
            "watch_top_n": top_n,
            "research_watch_only": True,
            "buy_signal": False,
    ...
    _write_json(out / "source_coverage.json", source_coverage(surface, watch_rows, surface_root, source_cov))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--surface-root", type=Path, default=DEFAULT_FORWARD_SURFACE_ROOT)
