...
AXIS_ID = "tradex_downside_prebreak_narrow_universe_selection_v4"
SCHEMA_PREFIX = "tradex_downside_prebreak_narrow_universe_selection_v4"
DEFAULT_OUTPUT_DIR_NAME = "downside_prebreak_narrow_universe_selection_v4"
DEFAULT_TOP_K = 5
DEFAULT_MIN_TRAIN_MONTHS = 6
DEFAULT_LOOKBACK_MONTHS = 12
DEFAULT_BASELINE_RANK_LIMIT = 20

REQUIRED_ARTIFACTS = (
    "downside_prebreak_narrow_universe_contract.json",
    "downside_prebreak_narrow_universe_bucket_model.json",
    "downside_prebreak_narrow_universe_monthly_rankings.csv",
    "downside_prebreak_narrow_universe_compare.json",
    "downside_prebreak_narrow_universe_yearly_performance.json",
    "downside_prebreak_narrow_universe_decision.json",
...
def _row_label(row: dict[str, Any]) -> int:
    ret5 = _safe_float(row.get("short_ret_5"))
    ret10 = _safe_float(row.get("short_ret_10"))
    if ret5 is None or ret10 is None:
        raise ValueError("short_ret_5 and short_ret_10 are required for prebreak-event labeling")
    return 1 if ret5 >= 0.025 or ret10 >= 0.04 else 0
...
def _acceleration_target(row: dict[str, Any]) -> float:
    ...
    if ret5 is None or ret10 is None or ret20 is None:
        raise ValueError("short_ret_5, short_ret_10, and short_ret_20 are required for prebreak-event target")
    return 0.65 * ret5 + 0.25 * ret10 + 0.10 * ret20


def _is_closed_horizon(row: dict[str, Any]) -> bool:
    return (
        _safe_float(row.get("short_ret_5")) is not None
        and _safe_float(row.get("short_ret_10")) is not None
        and _safe_float(row.get("short_ret_20")) is not None
    )


def _in_narrow_universe(row: dict[str, Any], *, baseline_rank_limit: int) -> bool:
    rank = _safe_int(row.get("baseline_rank"))
    return rank is not None and 1 <= rank <= baseline_rank_limit


def _baseline_sort_key(row: dict[str, Any]) -> tuple[Any, ...]:
    return (
        row.get("baseline_rank") is None,
        int(row.get("baseline_rank") or 10**9),
        -(float(row.get("tradePriorityScore") or 0.0)),
        -(float(row.get("entryScore") or 0.0)),
        str(row.get("code")),
    )


def _baseline_neighborhood_rows(rows: list[dict[str, Any]], *, limit: int) -> list[dict[str, Any]]:
    return sorted([row for row in rows if _is_closed_horizon(row)], key=_baseline_sort_key)[:limit]
...
def _performance_summary(rankings: list[dict[str, Any]], rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    baseline_event_labels = [_row_label(row) for row in baseline_rows]
    challenger_event_labels = [_row_label(row) for row in challenger_rows]
    baseline_targets = [_acceleration_target(row) for row in baseline_rows]
    challenger_targets = [_acceleration_target(row) for row in challenger_rows]
    baseline_event_rate = (sum(baseline_event_labels) / len(baseline_event_labels)) if baseline_event_labels else None
    challenger_event_rate = (sum(challenger_event_labels) / len(challenger_event_labels)) if challenger_event_labels else None
    return {
        "baseline": {
            ...
            "prebreak_event_rate": baseline_event_rate,
            "near_term_target_mean": _mean(baseline_targets),
        },
        "challenger": {
            ...
            "prebreak_event_rate": challenger_event_rate,
            "near_term_target_mean": _mean(challenger_targets),
        },
        "delta": {
            ...
            "prebreak_event_rate_delta": None if baseline_event_rate is None or challenger_event_rate is None else challenger_event_rate - baseline_event_rate,
            "near_term_target_mean_delta": None if _mean(baseline_targets) is None or _mean(challenger_targets) is None else _mean(challenger_targets) - _mean(baseline_targets),
...
def _decision(compare: dict[str, Any], rankings: list[dict[str, Any]]) -> tuple[str, str]:
    ...
    if (
        chal.get("prebreak_event_rate") is not None
        and base.get("prebreak_event_rate") is not None
        and chal["prebreak_event_rate"] > base["prebreak_event_rate"]
        and chal["near_term_target_mean"] > base["near_term_target_mean"]
        and chal["mean_ret20"] >= base["mean_ret20"] - 0.01
        and monthly["positive_months"] >= monthly["negative_months"]
    ):
        return "keep_for_shadow_paper_replay", "prebreak_event_selector_improves_same_condition_near_term_breakdown_quality"
    if chal.get("prebreak_event_rate") is not None and base.get("prebreak_event_rate") is not None and chal["prebreak_event_rate"] <= base["prebreak_event_rate"]:
        return "drop_as_prebreak_event_edge_insufficient", "same_condition_event_quality_failed"
...
def run_pipeline(*, db_path: str | Path | None = None, output_dir: str | Path | None = None, start_ymd: int = 20240101, end_ymd: int = 20260515, top_k: int = DEFAULT_TOP_K, min_train_months: int = DEFAULT_MIN_TRAIN_MONTHS, lookback_months: int = DEFAULT_LOOKBACK_MONTHS, baseline_rank_limit: int = DEFAULT_BASELINE_RANK_LIMIT, prebuilt_rows: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    source_db = base._resolve_db_path(db_path)
    output_root = Path(output_dir).expanduser().resolve() if output_dir else Path(r"G:\Tradex\downside_prebreak_narrow_universe_selection_v4")
    ...
    if prebuilt_rows is None:
        ...
        rows = [dict(row) for row in prebuilt_rows]
    unknown_rows = [row for row in rows if not _is_closed_horizon(row)]
    by_all_month: dict[int, list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
        by_all_month[_month_key(row["ymd"])].append(row)
    candidate_rows = [row for month_rows in by_all_month.values() for row in _baseline_neighborhood_rows(month_rows, limit=baseline_rank_limit)]
    candidate_ids = {f"{row['ymd']}|{row['code']}" for row in candidate_rows}
    out_of_narrow_universe_rows = [row for row in rows if _is_closed_horizon(row) and f"{row['ymd']}|{row['code']}" not in candidate_ids]
    by_month: dict[int, list[dict[str, Any]]] = defaultdict(list)
    ...
    for month in sorted(by_month):
        ...
        if len(train_rows) < top_k or len(month_rows) < top_k:
            monthly_rankings.append(
                {
                    "month": int(month),
                    "train_month_count": len(train_months),
                    "train_row_count": len(train_rows),
                    "train_label_totals": {"positive": 0, "negative": 0},
                    "baseline_top_codes": [],
                    "challenger_top_codes": [],
                    "baseline_top_ids": [],
                    "challenger_top_ids": [],
                    "baseline_top_hit_rate": None,
                    "challenger_top_hit_rate": None,
                    "baseline_top_mean_ret20": None,
                    "challenger_top_mean_ret20": None,
                    "baseline_top_median_ret20": None,
                    "challenger_top_median_ret20": None,
                    "changed_top5_members_count": 0,
                    "changed_rank_count": 0,
                    "selected_candidate_count": 0,
                    "closed_horizon_candidate_count": len(month_rows),
                    "unknown_candidate_count": sum(1 for row in rows if _month_key(row["ymd"]) == month and not _is_closed_horizon(row)),
                    "out_of_narrow_universe_count": sum(1 for row in rows if _month_key(row["ymd"]) == month and _is_closed_horizon(row) and f"{row['ymd']}|{row['code']}" not in candidate_ids),
                    "skipped": True,
                    "skip_reason": "insufficient_closed_horizon_rows",
                }
            )
            continue
        ...
        month_rank = {
            ...
            "closed_horizon_candidate_count": len(month_scored),
            "unknown_candidate_count": sum(1 for row in rows if _month_key(row["ymd"]) == month and not _is_closed_horizon(row)),
            "out_of_narrow_universe_count": sum(1 for row in rows if _month_key(row["ymd"]) == month and _is_closed_horizon(row) and f"{row['ymd']}|{row['code']}" not in candidate_ids),
            "skipped": False,
        }
    ...
    contract = {
        ...
        "objective": "near-term downside pre-break event short selector in baseline-ranked neighborhood",
        "selection_universe": f"closed-horizon short candidates in monthly baseline-sorted top {baseline_rank_limit}",
        "selection_policy": {
            "walk_forward": True,
            "top_k": int(top_k),
            "min_train_months": int(min_train_months),
            "lookback_months": int(lookback_months),
            "baseline_rank_limit": int(baseline_rank_limit),
            "label": "known short_ret_5 >= 0.025 OR known short_ret_10 >= 0.04",
            "label_definition": "near-term breakdown event if the short side earns at least 2.5% within 5 sessions or 4.0% within 10 sessions; unknown horizons are excluded",
            "target_definition": "0.65 * known short_ret_5 + 0.25 * known short_ret_10 + 0.10 * known short_ret_20",
        },
    ...
    no_lookahead = {
        ...
        "unknown_horizon_rows_excluded_from_training": True,
        "unknown_horizon_rows_excluded_from_evaluation": True,
        "unknown_horizon_row_count": len(unknown_rows),
        "closed_horizon_row_count": len(candidate_rows),
        "out_of_narrow_universe_row_count": len(out_of_narrow_universe_rows),
    ...
    _write_json(output_root / "downside_prebreak_narrow_universe_contract.json", contract)
    _write_json(output_root / "downside_prebreak_narrow_universe_bucket_model.json", bucket_model)
    _write_csv(output_root / "downside_prebreak_narrow_universe_monthly_rankings.csv", pd.DataFrame(monthly_rankings))
    _write_json(output_root / "downside_prebreak_narrow_universe_compare.json", compare_json)
    _write_json(output_root / "downside_prebreak_narrow_universe_yearly_performance.json", {"schema_version": f"{SCHEMA_PREFIX}_yearly_performance_v1", "axis_id": AXIS_ID, "yearly": yearly_summary.to_dict(orient="records")})
    _write_json(output_root / "downside_prebreak_narrow_universe_decision.json", decision_payload)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Statistical downside pre-break event short selector in a baseline-ranked neighborhood.")
    ...
    parser.add_argument("--baseline-rank-limit", type=int, default=DEFAULT_BASELINE_RANK_LIMIT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = run_pipeline(
        ...
        baseline_rank_limit=args.baseline_rank_limit,
    )
