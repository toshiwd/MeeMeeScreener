...
AXIS_ID = "context_risk_not_clean_stability_replay_v1"
DEFAULT_SOURCE = Path("G:/Tradex/context_risk_not_clean_candidate_pretest_v1/20260604T024237Z-context-risk-not-clean-candidate-pretest-v1/final_research_decision.json")
DEFAULT_EXPANDED_ROWS = Path("G:/Tradex/intersection_family_current_period_risk_containment_v1/20260526T010028Z-intersection-family-current-period-risk-containment-v1/risk_containment_rows.csv")
DEFAULT_OUT_ROOT = Path("G:/Tradex/context_risk_not_clean_stability_replay_v1")
TOPKS = (5, 10, 20)
REQUIRED = (
    "final_research_decision.json",
    "stability_replay_compare.json",
    "stability_effect_summary.json",
...
def _load_rows(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path)
    required = {"as_of_date", "code", "fresh_runtime_research_watch_rank", "ret20", "period_bucket", "variant_c_combined_context_risk_clean"}
    missing = sorted(required - set(df.columns))
    if missing:
        raise RuntimeError(f"expanded replay source missing required columns: {missing}")
...
def _compare(df: pd.DataFrame) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    period_stability = []
    for period, part in df.groupby("period_bucket", sort=True):
        for topk in (5, 10):
            b, c = part[part["baseline_top_rank"] <= topk], part[part["challenger_rank"] <= topk]
            period_stability.append({
                ...
                "hit_rate_delta": (_rate(c["hit_flag"]) - _rate(b["hit_flag"])) if _rate(c["hit_flag"]) is not None and _rate(b["hit_flag"]) is not None else None,
                ...
                "severe_loss_rate_delta": (_rate(c["severe_loss_flag"]) - _rate(b["severe_loss_flag"])) if _rate(c["severe_loss_flag"]) is not None and _rate(b["severe_loss_flag"]) is not None else None,
            })
    return {"metrics": metrics, "deltas": deltas, "period_stability": period_stability}, effect_summary, repl_summary
...
def _decision(compare: dict[str, Any], effect: dict[str, Any]) -> tuple[str, str, bool]:
    ...
    positive = (top5["mean_ret20_delta"] or 0) >= 0 and (top10["mean_ret20_delta"] or 0) >= 0 and ((top10["bad_pick_rate_delta"] or 0) < 0 or (top10["severe_loss_rate_delta"] or 0) < 0)
    no_cost = (top5["hit_rate_delta"] or 0) >= -0.01 and (top10["hit_rate_delta"] or 0) >= -0.01
    meaningful_branching = e10["changed_members_count"] >= 10 and e10["dates_with_member_branching"] >= 5
    stable_periods = [p for p in compare["period_stability"] if p["topk"] == 10 and p["row_count"] >= 30]
    period_positive = stable_periods and sum(1 for p in stable_periods if (p["mean_ret20_delta"] or 0) >= 0 and (p["bad_pick_rate_delta"] or 0) <= 0) >= max(1, math.ceil(len(stable_periods) * 0.6))
    if positive and no_cost and meaningful_branching and period_positive:
        return "keep", "stability_replay_reproduced_improvement_with_meaningful_branching_and_no_unacceptable_cost", True
    if positive and no_cost:
        return "hold", "direction_positive_but_branching_or_period_stability_still_insufficient", True
    return "drop", "stability_replay_failed_to_reproduce_topk_quality_improvement", False


def run(args: argparse.Namespace) -> Path:
    source = _read_json(args.source_artifact)
    if source.get("authoritative_rollup_decision") != "hold":
        raise RuntimeError("source pretest decision is not hold")
    out_dir = args.out_root / f"{_now_tag()}-{AXIS_ID.replace('_', '-')}"
    out_dir.mkdir(parents=True, exist_ok=False)
    df = _load_rows(args.expanded_rows)
    ...
    tested = {
        ...
        "form": "demotion",
        "rule": "clean rows first, context_risk_not_clean rows later within same as_of_date, preserving baseline rank inside each group",
        "threshold_tuning": False,
    }
    stability_scope = {
        "scope_type": "expanded_replay_existing_contract",
        "expanded_rows": str(args.expanded_rows),
        "row_count": int(len(df)),
        "unique_date_count": int(df["as_of_date"].nunique()),
        "unique_symbol_count": int(df["code"].nunique()),
        "source_contract_changed": False,
        "baseline_rank_source_logic_changed": False,
        "note": "Used existing risk_containment_rows because it already carries the fixed deployable flag and replay outcomes.",
    }
    _write_json(out_dir / "stability_replay_compare.json", {"axis_id": AXIS_ID, "tested_challenger": tested, "stability_scope": stability_scope, "compare": compare})
    _write_json(out_dir / "stability_effect_summary.json", {"axis_id": AXIS_ID, **effect})
    ...
    final = {
        ...
        "next_required_single_step": "keep_research_candidate_and_prepare_next_stage_comparison_only_if_user_approves" if decision == "keep" else ("do_not_promote_expand_stability_or_contract_before_next_stage" if decision == "hold" else "drop_context_risk_not_clean_candidate"),
        "boundary_flags": {
            "tradex_only": True,
            "research_stability_check_only": True,
            ...
            "threshold_tuning": False,
            "challenger_design_changed": False,
            "baseline_rank_source_logic_changed": False,
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stability replay for fixed context_risk_not_clean demotion challenger.")
    parser.add_argument("--source-artifact", type=Path, default=DEFAULT_SOURCE)
    parser.add_argument("--expanded-rows", type=Path, default=DEFAULT_EXPANDED_ROWS)
