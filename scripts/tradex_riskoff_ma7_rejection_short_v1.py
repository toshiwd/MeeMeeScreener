...
AXIS_ID = "tradex_riskoff_ma7_rejection_short_v1"
OUT = Path(r"G:\Tradex\riskoff_ma7_rejection_short_v1")
...
def run() -> Path:
    ...
    leads = ",".join(
        [f"lead(h,{i}) over w h{i},lead(l,{i}) over w l{i},lead(c,{i}) over w c{i},lead(date,{i}) over w d{i}" for i in range(1, 11)]
    )
    sql = f"""
    WITH b AS (
      ...
      SELECT b.*,breadth_above_ma20,
        row_number() OVER(PARTITION BY b.date ORDER BY (c-l)/nullif(h-l,0),code) day_rank
      FROM b JOIN breadth USING(date)
      WHERE d10 IS NOT NULL AND ma7<ma20 AND ma20<ma60 AND c<ma7 AND c<o
        AND h>=ma7*.99 AND h<=ma7*1.03 AND (c-l)/nullif(h-l,0)<=.30
    ...
    for item in raw.to_dict("records"):
        ...
        rows.append({
            ...
            "ret": ret, "rule": "riskoff_ma7_rejection_short", "regime": "risk_off",
    ...
    dev = reports["development_2019_2025"]["metrics"]
    ...
    payload = {
        ...
        "fixed_evaluation_conditions": {
            "shape": "MA7<MA20<MA60; intraday high retests MA7 -1%/+3%; bearish close back below MA7; close in bottom30%",
            "regime": "breadth above MA20 <=45%", "entry": "next open only when open>=signal close",
            "exit": "short TP10 SL5 H10 stop-first", "ranking": "bottom close position top3/day", "costs": "ignored",
        },
        "reports": reports, "decision": {"candidate_local_decision": "keep" if keep else "drop", "authoritative_rollup_decision": "research_only", "reason_type": "fixed_riskoff_ma7_rejection_gate_pass" if keep else "fixed_gate_failed"},
