...
AXIS_ID = "fresh_watch_outcome_evaluation_v1"
...
DEFAULT_AVAILABILITY_ROOT = Path(r"G:\Tradex\fresh_watch_outcome_availability_audit_v1\20260525T144138Z-fresh-watch-outcome-availability-audit-v1")
DEFAULT_SOURCE_DB = Path(r"C:\Users\enish\AppData\Local\MeeMeeScreener\data\stocks.duckdb")
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\fresh_watch_outcome_evaluation_v1")
REQUIRED_ARTIFACTS = (
    "fresh_outcome_evaluation_summary.json",
    "fresh_outcome_evaluation_rows.csv",
    "fresh_watch_bucket_metrics.json",
...
def no_lookahead_audit(rows: pd.DataFrame, watch_decision: dict[str, Any]) -> dict[str, Any]:
    frozen_ok = watch_decision.get("research_decision") == "fresh_runtime_watch_protocol_ready_for_future_outcome_validation"
    outcome_after_freeze = "ret20" in rows.columns and "fresh_watch_protocol_rank" in rows.columns
...
def decide(metrics: dict[str, Any], gate: dict[str, Any], audit: dict[str, Any]) -> tuple[str, str, list[str]]:
    if not audit["no_lookahead_pass"]:
        return "blocked_no_lookahead_violation", "BLOCKED", ["fresh_watch_outcome_evaluation_failed_no_lookahead_audit"]
    if metrics["outcome_coverage_rate"] < 1.0:
        return "fresh_watch_outcome_evaluation_pending_more_confirmed_bars", "HOLD_UNDERPOWERED", ["ret20_outcomes_not_available_for_all_fresh_watch_rows"]
    if gate["buyability_gate_pass"]:
        return "fresh_watch_buyability_gate_passed_for_next_validation", "KEEP", ["fresh_watch_rows_passed_predeclared_forward_outcome_gate"]
    return "fresh_watch_buyability_gate_failed", "DROP", ["fresh_watch_rows_failed_predeclared_forward_outcome_gate"]
...
def run(
    ...
    out = output_root / f"{_now_tag()}-fresh-watch-outcome-evaluation-v1"
    out.mkdir(parents=True, exist_ok=True)
    evaluated.to_csv(out / "fresh_outcome_evaluation_rows.csv", index=False)
    _write_json(out / "fresh_outcome_evaluation_summary.json", {"axis_id": AXIS_ID, "decision": decision, "decision_class": decision_class, "reason_typed": reasons, "watch_root": str(watch_root), "availability_root": str(availability_root), "source_db": str(source_db), "buyable_selection_ready": decision_class == "KEEP", "validated_buy_count": 0, **metrics})
    _write_json(out / "fresh_watch_bucket_metrics.json", {"axis_id": AXIS_ID, "fresh_watch_top20": metrics})
