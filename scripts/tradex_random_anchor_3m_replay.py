...
def _sample_anchor_dates(
    ...
    *,
    ...
    sampled_months = rng.sample(eligible_months, min(int(anchor_count), len(eligible_months)))
    sampled_months.sort()
...
def _build_selection_rows_and_policy_runs(
    *,
    ...
    up_snapshot = _ranking_snapshot(as_of=anchor_spec.anchor_date, direction="up", limit=pool_limit)
    down_snapshot = _ranking_snapshot(as_of=anchor_spec.anchor_date, direction="down", limit=pool_limit)
    ...
    for side, symbol_list, baseline_rows, challenger_rows in (
        ...
        symbol_list = [symbol for symbol in symbol_list if symbol in available_symbols]
        if not symbol_list:
            continue
    ...
    return candidate_rows, selection_rows, policy_run_rows, policy_ledger_rows
...
def run_random_anchor_replay(
    *,
    ...
    basis_start_dt, basis_end_dt = _load_signal_basis_bounds(source_db_path=source_db_path)
    ...
    for anchor_spec in anchors:
        ...
        candidate_rows_anchor, selection_rows_anchor, policy_run_rows_anchor, policy_ledger_rows_anchor = _build_selection_rows_and_policy_runs(
            source_db_path=source_db_path,
            anchor_spec=anchor_spec,
    ...
    summary_payload = {
        ...
        "side_metrics": side_metrics,
        "paths": {
            "random_anchor_dates_json": str(output_dir / "random_anchor_dates.json"),
            "random_anchor_candidate_snapshots_json": str(output_dir / "random_anchor_candidate_snapshots.json"),
            "selection_only_replay_ledger_json": str(output_dir / "selection_only_replay_ledger.json"),
            "selection_only_replay_ledger_parquet": str(output_dir / "selection_only_replay_ledger.parquet"),
            "policy_trade_replay_ledger_json": str(output_dir / "policy_trade_replay_ledger.json"),
            "policy_trade_replay_ledger_parquet": str(output_dir / "policy_trade_replay_ledger.parquet"),
            "champion_vs_challenger_random_anchor_compare_json": str(output_dir / "champion_vs_challenger_random_anchor_compare.json"),
            "random_anchor_replay_summary_json": str(output_dir / "random_anchor_replay_summary.json"),
        },
    ...
    _write_payload_json(output_dir / "random_anchor_dates.json", random_anchor_dates_payload)
    _write_payload_json(output_dir / "random_anchor_candidate_snapshots.json", {
    ...
    _write_payload_json(output_dir / "selection_only_replay_ledger.json", {
        "schema_version": "tradex_selection_only_replay_ledger_v1",
        "generated_at": _utc_now(),
        "rows": selection_rows,
    })
    if not selection_only_frame.empty:
        _write_parquet(output_dir / "selection_only_replay_ledger.parquet", selection_only_frame)
    else:
        _write_parquet(output_dir / "selection_only_replay_ledger.parquet", pd.DataFrame(selection_rows))
    _write_payload_json(output_dir / "policy_trade_replay_ledger.json", {
        "schema_version": "tradex_policy_trade_replay_ledger_v1",
        "generated_at": _utc_now(),
        "rows": policy_ledger_rows,
    })
    if not policy_ledger_frame.empty:
        _write_parquet(output_dir / "policy_trade_replay_ledger.parquet", policy_ledger_frame)
    else:
        _write_parquet(output_dir / "policy_trade_replay_ledger.parquet", pd.DataFrame(policy_ledger_rows))
    _write_payload_json(output_dir / "champion_vs_challenger_random_anchor_compare.json", compare_payload)
    _write_payload_json(output_dir / "random_anchor_replay_summary.json", summary_payload)
