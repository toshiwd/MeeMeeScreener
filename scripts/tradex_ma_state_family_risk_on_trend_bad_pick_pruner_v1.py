...
DEFAULT_SOURCE_FAMILY_SESSION = Path(r"G:\Tradex\ma_position_path_research_family_bad_pick_refinement\20260429T064544Z-bf5d13f6")
DEFAULT_CANDIDATE_INPUT_DIR = Path(r"G:\Tradex\sample_replays\tradex_integrated_guarded_v1_stress200")
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\ma_state_family_risk_on_trend_bad_pick_pruner_v1")
...
DEFAULT_PRIOR_PRUNER_SESSION = Path(r"G:\Tradex\ma_state_family_regime_only_bad_pick_pruner_v1\20260429T074051Z-0d1f3df7")
DEFAULT_LIMIT_ANCHOR_DATES = None

SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1"
COMPARE_SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1_compare_v1"
DECISION_SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1_decision_v1"
MANIFEST_SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1_manifest_v1"
MONTHLY_SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1_monthly_comparison_v1"
REGIME_SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1_regime_comparison_v1"
PENALTY_COVERAGE_SCHEMA_VERSION = "tradex_ma_state_family_risk_on_trend_bad_pick_pruner_v1_penalty_coverage_v1"

TOP_K_VALUES = (5, 10, 20)
RISK_ON_TREND_ONLY_PENALTY = -0.08
MAX_TOTAL_PENALTY = -0.08
...
def _progress_log(message: str) -> None:
    print(f"[ma_state_family_risk_on_trend_bad_pick_pruner_v1] {message}", file=sys.stderr, flush=True)
...
def _load_source_family_session(source_family_session: Path) -> dict[str, Any]:
    ...
    decision = _load_json(source_family_session / "state_family_bad_pick_surface_refinement_v1_decision.json")
    shadow = _load_json(source_family_session / "bad_pick_shadow_classification.json")
    compare = _load_json(source_family_session / "bad_pick_relaxation_compare.json")
    row_parquet = source_family_session / "bad_pick_refinement_rows.parquet"
    if not row_parquet.exists():
        raise FileNotFoundError(f"missing required source artifact: {row_parquet}")
    source_ma_manifest_path = Path(manifest["source_artifacts"]["source_ma_run_manifest_json"])
    source_ma_manifest = _load_json(source_ma_manifest_path)
    source_ma_row_parquet = Path(source_ma_manifest["output_artifacts"]["position_state_forward_path_rows_parquet"])
...
def _build_enriched_candidates(
    *,
    ...
    try:
        ...
        SELECT
            e.*,
            f.sample_count AS family_sample_count,
            f.unique_symbol_count AS family_unique_symbol_count,
            f.month_count AS family_month_count,
            f.mean_forward_ret_3d AS family_mean_forward_ret_3d,
            f.mean_forward_ret_5d AS family_mean_forward_ret_5d,
            f.mean_forward_ret_10d AS family_mean_forward_ret_10d,
            f.mean_forward_ret_20d AS family_mean_forward_ret_20d,
            f.median_forward_ret_20d AS family_median_forward_ret_20d,
            f.mean_mfe_20d AS family_mean_mfe_20d,
            f.mean_mae_20d AS family_mean_mae_20d,
            f.mean_path_value_score_v1 AS family_mean_path_value_score_v1,
            f.median_path_value_score_v1 AS family_median_path_value_score_v1,
            f.plus5_before_minus5_rate AS family_plus5_before_minus5_rate,
            f.minus5_before_plus5_rate AS family_minus5_before_plus5_rate,
            f.top15_rate AS family_top15_rate,
            f.bottom15_rate AS family_bottom15_rate,
            f.months_observed AS family_months_observed,
            f.positive_month_rate AS family_positive_month_rate,
            f.worst_month_mean_path_value AS family_worst_month_mean_path_value,
            f.best_month_mean_path_value AS family_best_month_mean_path_value,
            f.mean_monthly_path_value AS family_mean_monthly_path_value,
            f.std_monthly_path_value AS family_std_monthly_path_value,
            f.month_sample_count AS family_month_sample_count,
            f.regime_count AS family_regime_count,
            f.regime_consistency_score AS family_regime_consistency_score,
            f.score_spread AS family_score_spread,
            f.min_regime_mean_path_value AS family_min_regime_mean_path_value,
            f.max_regime_mean_path_value AS family_max_regime_mean_path_value,
            f.dominant_regime_context AS family_bad_pick_regime,
            f.dominant_regime_context AS dominant_regime_context,
            f.sample_qualified AS sample_qualified,
            f.core_bad_pick AS core_bad_pick,
            f.strict_bad_pick_family AS strict_bad_pick_family,
            f.relaxed_bad_pick_family AS relaxed_bad_pick_family,
            f.stability_hits_3 AS stability_hits_3,
            f.bad_pick_watch_family AS bad_pick_watch_family,
            f.regime_bad_pick_family AS regime_bad_pick_family,
            f.mae_risk_family AS mae_risk_family,
            f.endpoint_bad_pick_family AS endpoint_bad_pick_family,
            f.shadow_label_count AS shadow_label_count,
            f.source_family_session_id AS family_source_family_session_id,
            f.source_family_session_path AS family_source_family_session_path,
            f.source_row_count AS family_source_row_count,
            f.strict_relax_gap AS family_strict_relax_gap
        FROM enriched e
        LEFT JOIN read_parquet('{family_row_parquet.as_posix()}') f
          ON f.state_family_id = e.state_family_id
...
def _classify_penalty(row: pd.Series) -> tuple[float, list[str], bool]:
    row_regime = str(row.get("family_regime_context") or "")
    family_bad_pick_regime = str(row.get("family_bad_pick_regime") or "")
    regime_match = bool(row.get("regime_bad_pick_family")) and row_regime == family_bad_pick_regime
    if row_regime == "C:risk_on_trend" and regime_match:
        return float(RISK_ON_TREND_ONLY_PENALTY), ["regime_bad_pick_family", "risk_on_trend_match", "risk_on_trend_only"], True
    if regime_match:
        return 0.0, [], False
    return 0.0, [], False


def _apply_pruner_scores(frame: pd.DataFrame) -> pd.DataFrame:
    out = frame.copy()
    penalties = out.apply(_classify_penalty, axis=1, result_type="expand")
    out["score_adjustment"] = pd.to_numeric(penalties[0], errors="coerce").fillna(0.0)
    out["score_adjustment_reason"] = penalties[1].map(lambda value: list(value) if isinstance(value, list) else [])
    out["regime_match_penalty_applied"] = penalties[2].map(lambda value: bool(value))
...
def _aggregate_selected_rows(frame: pd.DataFrame, *, selected_col: str, bottom15_threshold: float) -> dict[str, Any]:
    ...
    bad_pick_family = selected["regime_bad_pick_family"].fillna(False).astype(bool)
    return {
        ...
        "regime_bad_pick_contamination_rate": float(selected["regime_bad_pick_family"].fillna(False).astype(bool).mean()),
...
def _build_compare_payload(
    *,
    ...
    for top_k in TOP_K_VALUES:
        ...
        champion_by_topk[str(top_k)] = _aggregate_selected_rows(frame, selected_col=champ_col, bottom15_threshold=bottom15_threshold)
        challenger_by_topk[str(top_k)] = _aggregate_selected_rows(frame, selected_col=chal_col, bottom15_threshold=bottom15_threshold)
    ...
    for month in months:
        ...
        if chal_sel.empty:
            zero_pass_months += 1
        champ_metric = _aggregate_selected_rows(month_frame, selected_col="champion_selected_top20", bottom15_threshold=bottom15_threshold)
        chal_metric = _aggregate_selected_rows(month_frame, selected_col="challenger_selected_top20", bottom15_threshold=bottom15_threshold)
    ...
    topk_summary = {
        ...
        "selection_divergence_reason": "risk_on_trend_only_bad_pick_pruner_v1_vs_champion",
    ...
    return {
        ...
        "penalty_formula": {
            "risk_on_trend_bad_pick_family_if_regime_matches": RISK_ON_TREND_ONLY_PENALTY,
            "family_bad_pick_regime_extraction": REGIME_BAD_PICK_REGIME_EXTRACTION,
            "max_total_penalty": MAX_TOTAL_PENALTY,
            "note": "risk_on_trend-only penalty uses dominant_regime_context extracted from the family row artifact",
        },
        "penalty_coverage": {},
...
def _build_monthly_comparison(frame: pd.DataFrame, *, bottom15_threshold: float) -> dict[str, Any]:
    ...
    for top_k in TOP_K_VALUES:
        ...
        for month in sorted(frame["month_bucket"].dropna().astype(str).unique().tolist()):
            month_frame = frame.loc[frame["month_bucket"].astype(str) == month].copy()
            champ = _aggregate_selected_rows(month_frame, selected_col=champ_col, bottom15_threshold=bottom15_threshold)
            chal = _aggregate_selected_rows(month_frame, selected_col=chal_col, bottom15_threshold=bottom15_threshold)
            rows.append(
                {
                    "month_bucket": month,
                    "champion": champ,
...
def _build_regime_comparison(frame: pd.DataFrame, *, bottom15_threshold: float) -> dict[str, Any]:
    by_topk: dict[str, Any] = {}
    ...
    for top_k in TOP_K_VALUES:
        ...
        for regime in sorted(frame[regime_col].fillna("unknown").astype(str).unique().tolist()):
            ...
            champ = _aggregate_selected_rows(regime_frame, selected_col=champ_col, bottom15_threshold=bottom15_threshold)
            chal = _aggregate_selected_rows(regime_frame, selected_col=chal_col, bottom15_threshold=bottom15_threshold)
            rows.append(
                {
...
def _build_penalty_coverage_summary(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    penalty_applied = int((frame["score_adjustment"] < 0).sum())
    regime_match_penalty = int(frame["regime_match_penalty_applied"].fillna(False).astype(bool).sum())
    ...
    penalized_rows_by_regime = {
        regime: int(((by_regime[regime_col] == regime) & (by_regime["score_adjustment"] < 0)).sum())
        for regime in sorted(by_regime[regime_col].unique().tolist())
    }
    shadow_counts = {
        "regime_bad_pick_family": int(frame["regime_bad_pick_family"].fillna(False).astype(bool).sum()),
    }
    return {
        ...
        "penalty_applied_rows": penalty_applied,
        "regime_match_penalty_rows": regime_match_penalty,
        "penalty_applied_rate": None if len(frame) == 0 else float(penalty_applied / len(frame)),
        "max_total_penalty": MAX_TOTAL_PENALTY,
        ...
        "penalized_rows_by_regime": penalized_rows_by_regime,
    }
...
def _build_decision_payload(
    *,
    ...
    if coverage_rate < 0.90:
        ...
        recommendation = "keep"
        reason = "bad_pick_pruner_reduces_contamination_without_material_path_regression"
    elif contamination_improved and (path_quality_flat_or_better or controlled_turnover):
        recommendation = "hold"
        reason = "contamination_improves_but_return_path_or_turnover_remains_mixed"
    else:
        recommendation = "drop"
        reason = "bad_pick_penalty_did_not_improve_topk_practical_quality"
    ...
    return {
        ...
        "family_bad_pick_regime_extraction": REGIME_BAD_PICK_REGIME_EXTRACTION,
        "family_bad_pick_regime_extraction_provenance": "provisional-derived from dominant_regime_context",
        "state_family_id_contract": "frozen from the verified family filter session; this pruner only changes score penalties",
        "penalty_formula": compare_payload["penalty_formula"],
...
def _build_v1_v1_1_risk_on_trend_delta_payload(
    *,
    ...
    return {
        ...
        "risk_on_trend_only_extraction": REGIME_BAD_PICK_REGIME_EXTRACTION,
...
def run_ma_state_family_risk_on_trend_bad_pick_pruner_v1(
    *,
    ...
    prior_pruner_session_path = Path(str(prior_pruner_session)).expanduser().resolve() if prior_pruner_session and str(prior_pruner_session).strip() else DEFAULT_PRIOR_PRUNER_SESSION.resolve()
    if not prior_pruner_session_path.exists():
        raise FileNotFoundError(f"prior pruner session not found: {prior_pruner_session_path}")
    ...
    prior_pruner_payloads = _load_prior_pruner_session(prior_pruner_session_path)
    ...
    thresholds = {
        "bottom15_score_threshold": _safe_float(source_payloads["decision"]["thresholds"].get("bottom15_score_threshold")),
        "top15_score_threshold": _safe_float(source_payloads["decision"]["thresholds"].get("top15_score_threshold")),
    ...
    enriched = _apply_pruner_scores(enriched)
    ...
    diff_rows["bad_pick_family_contaminant"] = diff_rows["regime_bad_pick_family"].fillna(False).astype(bool)
    ...
    monthly_payload = _build_monthly_comparison(diff_rows, bottom15_threshold=float(thresholds["bottom15_score_threshold"]))
    regime_payload = _build_regime_comparison(diff_rows, bottom15_threshold=float(thresholds["bottom15_score_threshold"]))
    ...
    output_files = {
        ...
        "ma_state_family_risk_on_trend_bad_pick_pruner_v1_compare_json": session_tmp / "ma_state_family_risk_on_trend_bad_pick_pruner_v1_compare.json",
        "ma_state_family_risk_on_trend_bad_pick_pruner_v1_decision_json": session_tmp / "ma_state_family_risk_on_trend_bad_pick_pruner_v1_decision.json",
        "prior_pruner_delta_json": session_tmp / "prior_pruner_delta.json",
        "topk_membership_diff_parquet": session_tmp / "topk_membership_diff.parquet",
        "penalty_coverage_summary_json": session_tmp / "penalty_coverage_summary.json",
    ...
    manifest_payload = {
        ...
        "prior_pruner_session_id": prior_pruner_session_path.name,
        "prior_pruner_session_path": str(prior_pruner_session_path),
        "candidate_input_dir": str(candidate_input_path),
        "candidate_input_json": str(candidate_input_path / "integrated_guarded_v1_candidate_snapshots.json"),
        "output_root": str(output_root_path),
        "source_artifacts": {
            "refinement_run_manifest_json": str(source_family_session_path / "run_manifest.json"),
            "refinement_shadow_classification_json": str(source_family_session_path / "bad_pick_shadow_classification.json"),
            "refinement_relaxation_compare_json": str(source_family_session_path / "bad_pick_relaxation_compare.json"),
            "refinement_decision_json": str(source_family_session_path / "state_family_bad_pick_surface_refinement_v1_decision.json"),
            "refinement_rows_parquet": str(source_family_session_path / "bad_pick_refinement_rows.parquet"),
            "source_ma_run_manifest_json": str(source_payloads["source_ma_session_path"] / "run_manifest.json"),
            "source_ma_row_parquet": str(source_payloads["source_ma_row_parquet"]),
            ...
            "prior_pruner_compare_json": str(prior_pruner_session_path / "ma_state_family_regime_only_bad_pick_pruner_v1_compare.json"),
            "prior_pruner_decision_json": str(prior_pruner_session_path / "ma_state_family_regime_only_bad_pick_pruner_v1_decision.json"),
            "prior_pruner_penalty_coverage_summary_json": str(prior_pruner_session_path / "penalty_coverage_summary.json"),
        },
        ...
        "score_adjustment_cap": MAX_TOTAL_PENALTY,
        "family_bad_pick_regime_extraction": REGIME_BAD_PICK_REGIME_EXTRACTION,
        "family_bad_pick_regime_extraction_provenance": "provisional-derived from dominant_regime_context",
    }

    prior_pruner_delta_payload = _build_v1_v1_1_risk_on_trend_delta_payload(
        current_compare_payload=compare_payload,
        current_decision_payload=decision_payload,
        current_coverage_payload=coverage_payload,
        v1_compare_payload=v1_payloads["compare"],
        v1_decision_payload=v1_payloads["decision"],
        v1_coverage_payload=v1_payloads["coverage"],
        v1_session_id=v1_session_path.name,
        v1_session_path=v1_session_path,
        v1_1_compare_payload=v1_1_payloads["compare"],
        v1_1_decision_payload=v1_1_payloads["decision"],
        v1_1_coverage_payload=v1_1_payloads["coverage"],
        v1_1_session_id=v1_1_session_path.name,
        v1_1_session_path=v1_1_session_path,
    )
    prior_pruner_compare_payload = prior_pruner_payloads["compare"]
    prior_pruner_decision_payload = prior_pruner_payloads["decision"]
    prior_pruner_coverage_payload = prior_pruner_payloads["coverage"]

    def _current_topk_entry(top_k: str) -> dict[str, Any]:
        return compare_payload["champion_vs_challenger"]["selection_only"][top_k]

    def _prior_topk_entry(top_k: str) -> dict[str, Any]:
        return prior_pruner_compare_payload["champion_vs_challenger"]["selection_only"][top_k]

    prior_pruner_delta_payload.update(
        {
            "source_prior_pruner_session_id": prior_pruner_session_path.name,
            "source_prior_pruner_session_path": str(prior_pruner_session_path),
            "prior_pruner_recommendation": prior_pruner_decision_payload.get("recommendation"),
            "prior_pruner_authoritative_rollup_decision": prior_pruner_decision_payload.get("authoritative_rollup_decision"),
            "prior_pruner_penalty_coverage": {
                "candidate_rows": int(prior_pruner_coverage_payload.get("candidate_rows") or 0),
                "matched_family_rows": int(prior_pruner_coverage_payload.get("matched_family_rows") or 0),
                "penalty_applied_rows": int(prior_pruner_coverage_payload.get("penalty_applied_rows") or 0),
                "regime_match_penalty_rows": int(prior_pruner_coverage_payload.get("regime_match_penalty_rows") or 0),
            },
            "topk_deltas_vs_prior_pruner": {
                top_k: {
                    "mean_forward_ret_20d_delta": None
                    if _current_topk_entry(top_k)["delta"]["mean_forward_ret_20d"] is None
                    or _prior_topk_entry(top_k)["delta"]["mean_forward_ret_20d"] is None
                    else float(_current_topk_entry(top_k)["delta"]["mean_forward_ret_20d"] - _prior_topk_entry(top_k)["delta"]["mean_forward_ret_20d"]),
                    "median_forward_ret_20d_delta": None
                    if _current_topk_entry(top_k)["delta"]["median_forward_ret_20d"] is None
                    or _prior_topk_entry(top_k)["delta"]["median_forward_ret_20d"] is None
                    else float(_current_topk_entry(top_k)["delta"]["median_forward_ret_20d"] - _prior_topk_entry(top_k)["delta"]["median_forward_ret_20d"]),
                    "mean_path_value_score_v1_delta": None
                    if _current_topk_entry(top_k)["delta"]["mean_path_value_score_v1"] is None
                    or _prior_topk_entry(top_k)["delta"]["mean_path_value_score_v1"] is None
                    else float(_current_topk_entry(top_k)["delta"]["mean_path_value_score_v1"] - _prior_topk_entry(top_k)["delta"]["mean_path_value_score_v1"]),
                    "median_path_value_score_v1_delta": None
                    if _current_topk_entry(top_k)["delta"]["median_path_value_score_v1"] is None
                    or _prior_topk_entry(top_k)["delta"]["median_path_value_score_v1"] is None
                    else float(_current_topk_entry(top_k)["delta"]["median_path_value_score_v1"] - _prior_topk_entry(top_k)["delta"]["median_path_value_score_v1"]),
                    "bad_pick_family_contamination_rate_delta": None
                    if _current_topk_entry(top_k)["delta"]["bad_pick_family_contamination_rate"] is None
                    or _prior_topk_entry(top_k)["delta"]["bad_pick_family_contamination_rate"] is None
                    else float(_current_topk_entry(top_k)["delta"]["bad_pick_family_contamination_rate"] - _prior_topk_entry(top_k)["delta"]["bad_pick_family_contamination_rate"]),
                    "bottom15_contamination_rate_delta": None
                    if _current_topk_entry(top_k)["delta"]["bottom15_contamination_rate"] is None
                    or _prior_topk_entry(top_k)["delta"]["bottom15_contamination_rate"] is None
                    else float(_current_topk_entry(top_k)["delta"]["bottom15_contamination_rate"] - _prior_topk_entry(top_k)["delta"]["bottom15_contamination_rate"]),
                }
                for top_k in ("5", "10", "20")
            },
            "branching_vs_prior_pruner": {
                "changed_top5_members_count_delta": int(compare_payload["topk_summary"]["changed_top5_members_count"] - prior_pruner_compare_payload["topk_summary"]["changed_top5_members_count"]),
                "changed_top10_members_count_delta": int(compare_payload["topk_summary"]["changed_top10_members_count"] - prior_pruner_compare_payload["topk_summary"]["changed_top10_members_count"]),
                "changed_top20_members_count_delta": int(compare_payload["topk_summary"]["changed_top20_members_count"] - prior_pruner_compare_payload["topk_summary"]["changed_top20_members_count"]),
                "changed_rank_count_delta": int(compare_payload["topk_summary"]["changed_rank_count"] - prior_pruner_compare_payload["topk_summary"]["changed_rank_count"]),
                "top5_overlap_ratio_delta": None
                if compare_payload["topk_summary"]["top5_overlap_ratio"] is None or prior_pruner_compare_payload["topk_summary"]["top5_overlap_ratio"] is None
                else float(compare_payload["topk_summary"]["top5_overlap_ratio"] - prior_pruner_compare_payload["topk_summary"]["top5_overlap_ratio"]),
                "top10_overlap_ratio_delta": None
                if compare_payload["topk_summary"]["top10_overlap_ratio"] is None or prior_pruner_compare_payload["topk_summary"]["top10_overlap_ratio"] is None
                else float(compare_payload["topk_summary"]["top10_overlap_ratio"] - prior_pruner_compare_payload["topk_summary"]["top10_overlap_ratio"]),
                "top20_overlap_ratio_delta": None
                if compare_payload["topk_summary"]["top20_overlap_ratio"] is None or prior_pruner_compare_payload["topk_summary"]["top20_overlap_ratio"] is None
                else float(compare_payload["topk_summary"]["top20_overlap_ratio"] - prior_pruner_compare_payload["topk_summary"]["top20_overlap_ratio"]),
            },
            "monthly_vs_prior_pruner": {
                "win_month_count_delta": int(decision_payload["monthly_summary_top20"].get("win_month_count", 0) - prior_pruner_decision_payload["monthly_summary_top20"].get("win_month_count", 0)),
                "loss_month_count_delta": int(decision_payload["monthly_summary_top20"].get("loss_month_count", 0) - prior_pruner_decision_payload["monthly_summary_top20"].get("loss_month_count", 0)),
                "flat_month_count_delta": int(decision_payload["monthly_summary_top20"].get("flat_month_count", 0) - prior_pruner_decision_payload["monthly_summary_top20"].get("flat_month_count", 0)),
                "zero_pass_months_delta": int(decision_payload["monthly_summary_top20"].get("zero_pass_months", 0) - prior_pruner_decision_payload["monthly_summary_top20"].get("zero_pass_months", 0)),
                "worst_month_delta_mean_forward_ret_20d_delta": None
                if decision_payload["monthly_summary_top20"].get("worst_month_delta_mean_forward_ret_20d") is None
                or prior_pruner_decision_payload["monthly_summary_top20"].get("worst_month_delta_mean_forward_ret_20d") is None
                else float(
                    decision_payload["monthly_summary_top20"]["worst_month_delta_mean_forward_ret_20d"]
                    - prior_pruner_decision_payload["monthly_summary_top20"]["worst_month_delta_mean_forward_ret_20d"]
                ),
                "best_month_delta_mean_forward_ret_20d_delta": None
                if decision_payload["monthly_summary_top20"].get("best_month_delta_mean_forward_ret_20d") is None
                or prior_pruner_decision_payload["monthly_summary_top20"].get("best_month_delta_mean_forward_ret_20d") is None
                else float(
                    decision_payload["monthly_summary_top20"]["best_month_delta_mean_forward_ret_20d"]
                    - prior_pruner_decision_payload["monthly_summary_top20"]["best_month_delta_mean_forward_ret_20d"]
                ),
            },
            "regime_vs_prior_pruner": {
                "win_regime_count_delta": int(decision_payload["regime_summary_top20"].get("win_regime_count", 0) - prior_pruner_decision_payload["regime_summary_top20"].get("win_regime_count", 0)),
                "loss_regime_count_delta": int(decision_payload["regime_summary_top20"].get("loss_regime_count", 0) - prior_pruner_decision_payload["regime_summary_top20"].get("loss_regime_count", 0)),
                "flat_regime_count_delta": int(decision_payload["regime_summary_top20"].get("flat_regime_count", 0) - prior_pruner_decision_payload["regime_summary_top20"].get("flat_regime_count", 0)),
                "worst_regime_delta_mean_forward_ret_20d_delta": None
                if decision_payload["regime_summary_top20"].get("worst_regime_delta_mean_forward_ret_20d") is None
                or prior_pruner_decision_payload["regime_summary_top20"].get("worst_regime_delta_mean_forward_ret_20d") is None
                else float(
                    decision_payload["regime_summary_top20"]["worst_regime_delta_mean_forward_ret_20d"]
                    - prior_pruner_decision_payload["regime_summary_top20"]["worst_regime_delta_mean_forward_ret_20d"]
                ),
                "best_regime_delta_mean_forward_ret_20d_delta": None
                if decision_payload["regime_summary_top20"].get("best_regime_delta_mean_forward_ret_20d") is None
                or prior_pruner_decision_payload["regime_summary_top20"].get("best_regime_delta_mean_forward_ret_20d") is None
                else float(
                    decision_payload["regime_summary_top20"]["best_regime_delta_mean_forward_ret_20d"]
                    - prior_pruner_decision_payload["regime_summary_top20"]["best_regime_delta_mean_forward_ret_20d"]
                ),
            },
        }
    )

    _write_json(output_files["ma_state_family_risk_on_trend_bad_pick_pruner_v1_compare_json"], compare_payload)
    _write_json(output_files["ma_state_family_risk_on_trend_bad_pick_pruner_v1_decision_json"], decision_payload)
    _write_json(output_files["prior_pruner_delta_json"], prior_pruner_delta_payload)
    _write_json(output_files["penalty_coverage_summary_json"], coverage_payload)
    ...
    return {
        ...
        "prior_pruner_delta": prior_pruner_delta_payload,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run TRADEX C:risk_on_trend-only bad-pick pruner against the refined family map.")
    ...
    parser.add_argument("--prior-pruner-session", default=str(DEFAULT_PRIOR_PRUNER_SESSION))
    ...
    run_ma_state_family_risk_on_trend_bad_pick_pruner_v1(
        ...
        prior_pruner_session=args.prior_pruner_session,
