...
AXIS_ID = "tradex_riskoff_support_retest_failure_short_v1"
OUT = Path(r"G:\Tradex\riskoff_support_retest_failure_short_v1")
...
def run() -> Path:
    ...
    leads = ",".join([f"lead(h,{i}) over w h{i},lead(l,{i}) over w l{i},lead(c,{i}) over w c{i},lead(date,{i}) over w d{i}" for i in range(1, 11)])
    sql = f"""
    WITH b AS (
      ...
      SELECT date,avg(CASE WHEN c>ma20 THEN 1.0 ELSE 0.0 END) breadth_above_ma20 FROM b WHERE ma20 IS NOT NULL GROUP BY date
    ), pairs AS (
      ...
      WHERE s.d10 IS NOT NULL AND s.h>=k.broken_support*.98 AND s.h<=k.broken_support*1.03
        ...
        AND breadth_above_ma20<=.45 AND s.next_open>=s.c
    ), signals AS (
    ...
    for item in raw.to_dict("records"):
        ...
        rows.append({
            "side": "sell", "code": str(item["code"]), "signal_date": pd.to_datetime(int(item["date"]), unit="s"),
            "entry_date": pd.to_datetime(int(item["next_entry_date"]), unit="s"), "ret": ret,
            "rule": "riskoff_support_retest_failure_short", "regime": "risk_off", "exit_offset": exit_offset,
            "breadth_above_ma20": float(item["breadth_above_ma20"]), "broken_support": float(item["broken_support"]),
        })
    ...
    payload = {
        ...
        "fixed_evaluation_conditions": {
            "anchor": "close below prior20 low", "signal": "1-5 sessions later retest old support -2%/+3%, close back below, bearish bottom40%",
            "regime": "breadth above MA20 <=45%", "entry": "next open only when open>=signal close", "exit": "short TP10 SL5 H10 stop-first",
            "ranking": "bottom close position top3/day", "costs": "ignored",
        },
        "reports": reports, "decision": {"candidate_local_decision": "keep" if keep else "drop", "authoritative_rollup_decision": "research_only", "reason_type": "fixed_retest_failure_gate_pass" if keep else "fixed_gate_failed"},
