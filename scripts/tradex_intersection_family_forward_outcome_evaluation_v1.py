...
AXIS_ID = "intersection_family_forward_outcome_evaluation_v1"
...
DEFAULT_AVAILABILITY_ROOT = Path(
    r"G:\Tradex\intersection_family_forward_outcome_availability_audit_v1\20260526T005152Z-intersection-family-forward-outcome-availability-audit-v1"
)
DEFAULT_SOURCE_DB = Path(r"C:\Users\enish\AppData\Local\MeeMeeScreener\data\stocks.duckdb")
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\intersection_family_forward_outcome_evaluation_v1")
REQUIRED_ARTIFACTS = (
    "forward_outcome_evaluation_summary.json",
    "forward_outcome_evaluation_rows.csv",
    "forward_candidate_metrics.json",
...
def attach_forward_outcomes(forward_rows: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    for row in out.itertuples(index=False):
        ...
        ret5.append(float(future.iloc[4]["close"]) / base_close - 1.0 if base_close and len(future) >= 5 else None)
        ret20.append(float(future.iloc[19]["close"]) / base_close - 1.0 if base_close and len(future) >= 20 else None)
...
def no_lookahead_audit(rows: pd.DataFrame, forward_decision: dict[str, Any]) -> dict[str, Any]:
    frozen_ok = forward_decision.get("research_decision") == "intersection_family_forward_paper_candidates_frozen"
    outcome_after_freeze = "ret20" in rows.columns and "forward_paper_rank" in rows.columns
    passed = bool(frozen_ok and outcome_after_freeze)
    return {
        "audit_result": "pass" if passed else "blocked",
        "no_lookahead_pass": passed,
        "candidate_rows_frozen_before_outcome_join": frozen_ok,
...
def buyability_gate_audit(metrics: dict[str, Any]) -> dict[str, Any]:
    coverage_ok = metrics["outcome_coverage_rate"] == 1.0 and metrics["sample_count"] >= 4
    quality_ok = (
...
def decide(metrics: dict[str, Any], gate: dict[str, Any], audit: dict[str, Any]) -> tuple[str, str, list[str]]:
    if not audit["no_lookahead_pass"]:
        return "blocked_no_lookahead_violation", "BLOCKED", ["intersection_forward_evaluation_failed_no_lookahead_audit"]
    if metrics["outcome_coverage_rate"] < 1.0:
        return "intersection_forward_outcome_evaluation_pending_more_confirmed_bars", "HOLD_UNDERPOWERED", [
            "ret20_outcomes_not_available_for_all_intersection_forward_candidates"
        ]
    if gate["buyability_gate_pass"]:
        return "intersection_forward_buyability_gate_passed_for_next_validation", "KEEP", [
            "intersection_forward_candidates_passed_predeclared_forward_outcome_gate"
        ]
    return "intersection_forward_buyability_gate_failed", "DROP", ["intersection_forward_candidates_failed_predeclared_forward_outcome_gate"]
...
def run(
    ...
    evaluated = attach_forward_outcomes(forward_rows, bars)
    metrics = metric_payload(evaluated)
    audit = no_lookahead_audit(evaluated, forward_decision)
    gate = buyability_gate_audit(metrics)
    decision, decision_class, reasons = decide(metrics, gate, audit)

    out = output_root / f"{_now_tag()}-intersection-family-forward-outcome-evaluation-v1"
    out.mkdir(parents=True, exist_ok=True)
    evaluated.to_csv(out / "forward_outcome_evaluation_rows.csv", index=False)
    _write_json(
        out / "forward_outcome_evaluation_summary.json",
        {
            "axis_id": AXIS_ID,
            "decision": decision,
    ...
    _write_json(out / "forward_candidate_metrics.json", {"axis_id": AXIS_ID, "intersection_forward_candidates": metrics})
    ...
    _write_json(
        out / "lineage.json",
        {
            "axis_id": AXIS_ID,
            "forward_root": str(forward_root),
            "forward_research_decision": forward_decision,
            "availability_root": str(availability_root),
            "availability_decision": _load_json(availability_root / "research_decision.json") if (availability_root / "research_decision.json").exists() else None,
        },
    )
    _write_json(
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = run(args.forward_root, args.availability_root, args.source_db, args.output_root)
