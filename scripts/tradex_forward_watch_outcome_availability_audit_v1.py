...
AXIS_ID = "forward_watch_outcome_availability_audit_v1"
...
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\forward_watch_outcome_availability_audit_v1")
REQUIRED_ARTIFACTS = (
    "outcome_availability_summary.json",
    "watch_outcome_availability_rows.csv",
...
def build_availability_rows(watch_rows: pd.DataFrame, bars: pd.DataFrame) -> pd.DataFrame:
    ...
    ret5_available: list[bool] = []
    ret20_available: list[bool] = []
    latest_bar_dates: list[int | None] = []
    available_future_dates: list[str] = []
    ...
    for row in out.itertuples(index=False):
        ...
        ret5_available.append(len(dates) >= 5)
        ret20_available.append(len(dates) >= 20)
        latest_bar_dates.append(max(dates) if dates else None)
        available_future_dates.append(",".join(str(d) for d in dates[:20]))
    out["available_future_session_count"] = future_counts
    out["ret5_evaluation_ready"] = ret5_available
    out["ret20_evaluation_ready"] = ret20_available
    out["latest_available_future_bar_date"] = latest_bar_dates
    out["available_future_bar_dates"] = available_future_dates
...
def no_lookahead_audit(rows: pd.DataFrame, watch_decision: dict[str, Any]) -> dict[str, Any]:
    selection_cols = {"ret5", "ret10", "ret20", "winner_ret20_gt_10pct", "bad_ret20_lt_minus_5pct", "severe_ret20_lt_minus_10pct"}
    forbidden_present = sorted(selection_cols & set(rows.columns))
    source_ok = bool(watch_decision.get("research_decision") == "forward_watch_protocol_ready_for_future_outcome_validation")
    passed = not forbidden_present and source_ok
    return {
        ...
        "forbidden_outcome_columns_present": forbidden_present,
...
def decide(status: dict[str, Any], audit: dict[str, Any]) -> tuple[str, str, list[str]]:
    if not audit["no_lookahead_pass"]:
        return "blocked_no_lookahead_violation", "BLOCKED", ["watch_protocol_or_rows_failed_no_lookahead"]
    if status["ret20_all_rows_ready"]:
        return "forward_watch_ret20_outcomes_ready_for_evaluation", "HOLD_UNDERPOWERED", ["future_ret20_window_available_for_all_watch_rows"]
    if status["ret5_all_rows_ready"]:
        return "forward_watch_ret5_ready_ret20_pending", "HOLD_UNDERPOWERED", ["short_horizon_ready_but_starter_entry_ret20_pending"]
    return "forward_watch_outcomes_pending_more_confirmed_bars", "HOLD_UNDERPOWERED", ["insufficient_future_confirmed_sessions_for_ret5_ret20_evaluation"]
...
def run(watch_root: Path = DEFAULT_WATCH_ROOT, source_db: Path = DEFAULT_SOURCE_DB, output_root: Path = DEFAULT_OUTPUT_ROOT) -> Path:
    ...
    out = output_root / f"{_now_tag()}-forward-watch-outcome-availability-audit-v1"
    out.mkdir(parents=True, exist_ok=True)
    availability.to_csv(out / "watch_outcome_availability_rows.csv", index=False)
    summary = {
        "axis_id": AXIS_ID,
        "decision": decision,
    ...
    _write_json(out / "outcome_availability_summary.json", summary)
    ...
    _write_json(
        out / "future_evaluation_readiness.json",
        {
            ...
            "next_valid_action": "wait_for_more_confirmed_bars_then_run_forward_outcome_evaluation",
            "buyable_selection_ready": False,
            "validated_buy_count": 0,
        },
    )
