...
from scripts.tradex_c_phase_long_touch_bounce_additive_oos_v1 import (
    ...
    _build_c_phase_long_candidates,
...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/c_phase_long_touch_bounce_guarded_oos_v1")
MAX_LATEST_PRICE_POSITION = 0.66
MAX_ABS_MA20_DISTANCE = 0.01
...
def _pass_guard(event: dict[str, Any]) -> bool:
    position = event.get("latest_price_position_pct")
    ma20_distance = event.get("ma20_distance")
    if position is None or ma20_distance is None:
        return False
    return float(position) <= MAX_LATEST_PRICE_POSITION and abs(float(ma20_distance)) <= MAX_ABS_MA20_DISTANCE


def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-c_phase_long_touch_bounce_guarded_oos_v1"
    ...
    base_candidates = _build_c_phase_long_candidates(by_code, buy_codes_by_date=buy_codes_by_date, start_dt=start_dt, end_dt=end_dt)
    guarded_candidates = [{**event, "guard_status": "pass"} for event in base_candidates if _pass_guard(event)]
    rejected_candidates = [{**event, "guard_status": "reject"} for event in base_candidates if not _pass_guard(event)]
    selected_by_date = _select_one_per_date(guarded_candidates)
    ...
    coverage = {
        ...
        "base_outside_additive_candidate_count": len(base_candidates),
        "guarded_additive_candidate_count": len(guarded_candidates),
        "guard_reject_count": len(rejected_candidates),
        "guarded_additive_candidate_date_count": len({int(row["dt"]) for row in guarded_candidates}),
        ...
        "max_latest_price_position": MAX_LATEST_PRICE_POSITION,
        "max_abs_ma20_distance": MAX_ABS_MA20_DISTANCE,
    }
    decision = _oos_decision(oos_compare, monthly, coverage)
    result = {
        "schema_version": "tradex_c_phase_long_touch_bounce_guarded_oos_v1",
        ...
        "fixed_evaluation_conditions": {
            ...
            "changed_axis": "c_phase_long_touch_bounce_plus_high_position_far_ma20_guard",
            "selection_rule": "base C touch-bounce candidate, then latest_price_position_pct <= 0.66 and abs(ma20_distance) <= 0.01; at most one outside-gap candidate per buy-candidate date",
        ...
        "observed_branching": {
            ...
            "selection_divergence_reason": "historical_oos_outside_gap_c_phase_long_touch_bounce_candidate_added_only_when_high_position_far_ma20_guard_passes",
        },
        ...
        "remaining_risks": [
            "guard thresholds are first post-mortem cut from failed C-axis ledger",
    ...
    with ledger_path.open("w", encoding="utf-8") as fh:
        for event in [*guarded_candidates, *rejected_candidates]:
