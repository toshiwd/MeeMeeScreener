...
def metric(group: pd.DataFrame, denominator: int) -> dict:
    return {
        ...
        "upward_continuation_rate": float(group["post_confirm_upward"].mean()),
        "eventual_drop5_rate": float(group["post_confirm_drop5"].mean()),
        "unresolved_rate": float(group["post_confirm_unresolved"].mean()),
        "mean_close15_pct": float(group["post_confirm_close15_pct"].mean()),
    }
...
def main() -> None:
    ...
    for code, events in paths.groupby("code", sort=False):
        ...
        for event in events.itertuples():
            ...
            confirm_pos = rebound_pos + 1
            entry_pos = rebound_pos + 2
            end_pos = entry_pos + 14
            if end_pos >= len(bars):
                continue
            rebound = bars.iloc[rebound_pos]
            ...
            rows.append({
                **event._asdict(),
                "confirm_ymd": int(confirm["ymd"]),
                "entry_ymd": int(bars.iloc[entry_pos]["ymd"]),
                "higher_low_confirm": bool(float(confirm["l"]) > float(rebound["l"])),
                "post_confirm_entry_open": entry_open,
                "post_confirm_close15_pct": close15_pct,
                "post_confirm_drop5": drop5,
                "post_confirm_upward": upward,
                "post_confirm_unresolved": bool(not drop5 and not upward),
            })
    ...
    for period, group in ledger.groupby("period"):
        ...
        for state, state_group in group.groupby("higher_low_confirm"):
            state_rows.append({"period": period, "state": "安値切り上げ" if state else "安値切り上げなし", **metric(state_group, len(group))})
    for year, group in ledger.assign(year=ledger["signal_ymd"] // 10000).groupby("year"):
        base = metric(group, len(group))
        for state, state_group in group.groupby("higher_low_confirm"):
            row = {"year": int(year), "state": "安値切り上げ" if state else "安値切り上げなし", **metric(state_group, len(group))}
    ...
    dev_states = states[states["period"].eq("development")]
    eligible = dev_states[
    ...
    selected = None if eligible.empty else str(eligible.sort_values("upward_continuation_rate", ascending=False).iloc[0]["state"])
    ...
    result = {
        "schema_version": "tradex_short_denial_higher_low_confirm_v1.compare.v1",
        "artifact_role": "authoritative_short_denial_higher_low_confirm", "review_only": True,
        "fixed_conditions": {
            "population": "Core signal denied by +3% rebound before -5% drop",
            "changed_axis": "next-session low higher than rebound-session low only",
            "entry_time": "session after confirmation open", "outcome_horizon": 15,
            "upward": "no -5% low and session-15 close >= +3% from entry open",
            "decline": "15-session low <= -5% from entry open", "development": "2019-2023",
        ...
        "observed_branching": {"changed_top5_members_count": None, "changed_top10_members_count": None, "changed_rank_count": int(len(ledger[ledger["higher_low_confirm"]])) if selected == "安値切り上げ" else int(len(ledger[~ledger["higher_low_confirm"]])) if selected else 0, "selection_divergence_reason": "confirmation-state split only"},
        "judgment": {"candidate_local_decision": "keep" if keep else "drop", "session_aggregate_decision": "keep_higher_low_confirm" if keep else "drop_higher_low_confirm", "authoritative_rollup_decision": "keep_short_denial_higher_low_confirm_v1_review_only" if keep else "drop_short_denial_higher_low_confirm_v1", "reason_type": "development_selected_validation_lift_drop_retention_year_gates"},
        "not_changed": ["売りシグナル", "売り候補", "MeeMee", "ranking", "runtime DB", "production logic"],
    }
    ledger.to_parquet(output / "higher_low_confirm_ledger.parquet", index=False)
    baseline.to_parquet(output / "higher_low_confirm_baseline.parquet", index=False)
    states.to_parquet(output / "higher_low_confirm_state_metrics.parquet", index=False)
    yearly.to_parquet(output / "higher_low_confirm_yearly_metrics.parquet", index=False)
