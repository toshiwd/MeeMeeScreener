...
FROZEN_FAILURE_SESSION_ID = "entry-short-broad-down-monthly-fix-20260419-131348"
...
RELAXED_VARIANT = trend.TrendVariant(
    name="broad_down_monthly_rangeprob_relaxed_v1",
    target="bad-pick removal",
    weekly_breakout_down_min=0.66,
    monthly_breakout_down_min=0.66,
    monthly_range_prob_max=0.35,
    close_pos_max=0.20,
)
...
def _build_reentry_audit(
    *,
    ...
    frozen_map = {_row_key(r): r for r in frozen_rows}
    ...
    return {
        "schema_version": "tradex_entry_precision_short_broad_down_rangeprob_reentry_audit_v1",
        "session_id": f"entry-short-broad-down-rangeprob-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}",
        ...
        "conclusion": {
            "monthly_range_prob_main_shared_blocker": False,
            "reason": "monthly_range_prob_changes_do_not_recover_the_removed_profitable_followthrough_shorts_in_broad_down",
        },
    }
...
def _monthly_ablation_compare(
    *,
    ...
    return {
        "schema_version": "tradex_entry_precision_short_broad_down_rangeprob_ablation_compare_v1",
        "session_id": f"entry-short-broad-down-rangeprob-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}",
        "generated_at": datetime.now(timezone.utc).isoformat(),
...
def _decision(quality_compare: dict[str, Any], reentry_audit: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    reasons = [
        ...
        f"relaxed_count={relaxed['metrics']['count']}",
        f"off_count={off['metrics']['count']}",
    ...
    if reentry_audit["conclusion"]["monthly_range_prob_main_shared_blocker"]:
        return "keep", reasons + ["monthly_range_prob_fix_succeeded"]
    return "drop", reasons + ["monthly_range_prob_not_primary_cause"]
...
def _write_report_md(path: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- monthly rangeprob variants tested: `{', '.join(v['name'] for v in compare['variants'])}`",
        "",
        "## Verify",
        f"- frozen challenger hit rate: `{compare['frozen_challenger']['hit_rate']}`",
        f"- relaxed profitable reentries: `{reentry['reentry_summary'][RELAXED_VARIANT.name]['profitable_removed_reentered']}`",
        f"- off profitable reentries: `{reentry['reentry_summary'][OFF_VARIANT.name]['profitable_removed_reentered']}`",
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    contract_payload = {
        "schema_version": "tradex_entry_precision_short_broad_down_rangeprob_contract_v1",
        ...
        "monthly_rangeprob_variants": [_variant_contract(RELAXED_VARIANT), _variant_contract(OFF_VARIANT)],
        "comparison_contract": {
            "same_universe": True,
            "same_period": True,
            "same_top_k": True,
    ...
    decision_payload = {
        "schema_version": "tradex_entry_precision_short_broad_down_rangeprob_fix_decision_v1",
        "session_id": session_id,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "baseline_id": BASELINE_ID,
        "challenger_id": CHALLENGER_ID,
        ...
        "confirmed": [
            ...
            "monthly_rangeprob_only_ablation",
            "json_is_authoritative",
        ],
        "provisional": [
            "trend_alignment_is_still_proxy_based",
            "regime_labels_are_proxy_derived",
            "monthly_rangeprob_is_only_one_part_of_the_broad_down_filter",
        ],
        "remaining_risks": [
    ...
    report_payload = {
        "current_state": {
            "confirmed": [
                ...
                "monthly_alignment_was_rejected_as_the_main_cause",
            ],
            "provisional": [
                "monthly_range_prob_is_the_next_smallest_suspect",
                "broad_down_sample_is_not_large",
                "regime_labels_are_proxy_derived",
            ],
        },
        "problem": "The broad-down failure may be caused by the monthly range-probability gate, but that needs isolated evidence before touching close-position or midrange logic.",
        "change_policy": "TRADEX research only, monthly_range_prob_max ablation only, keep long logic frozen, keep MeeMee untouched, keep close_pos_max and require_midrange_off frozen, and do not review other failure regimes.",
        "frozen_failure_session_id": FROZEN_FAILURE_SESSION_ID,
        ...
        "next_one_thing": "If the range-prob gate is not the culprit, the next single axis would be close_pos_max, not a broader redesign.",
    }

    _write_json(out_dir / "entry_precision_short_broad_down_rangeprob_contract.json", contract_payload)
    _write_json(out_dir / "entry_precision_short_broad_down_rangeprob_ablation_compare.json", quality_compare)
    _write_json(out_dir / "entry_precision_short_broad_down_rangeprob_reentry_audit.json", reentry_audit)
    _write_json(out_dir / "entry_precision_short_broad_down_rangeprob_quality_compare.json", quality_compare)
    _write_json(out_dir / "entry_precision_short_broad_down_rangeprob_fix_decision.json", decision_payload)
    _write_report_md(out_dir / "entry_precision_short_broad_down_rangeprob_fix_report.md", report_payload)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Monthly range-prob ablation for the frozen broad-down failure lane.")
