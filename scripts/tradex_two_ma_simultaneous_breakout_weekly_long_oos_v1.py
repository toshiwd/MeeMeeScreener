...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/two_ma_simultaneous_breakout_weekly_long_oos_v1")
...
def _to_date(ymd: int) -> datetime:
    return datetime.strptime(str(int(ymd)), "%Y%m%d")


def _weekly_bars(daily_bars: list[dict[str, Any]]) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    current_key: tuple[int, int] | None = None
    current_rows: list[dict[str, Any]] = []
    for row in daily_bars:
        key = _to_date(int(row["ymd"])).isocalendar()[:2]
        if current_key is None:
            current_key = key
        if key != current_key:
            out.append(_aggregate_week(current_rows))
            current_rows = []
            current_key = key
...
def _aggregate_week(rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "ymd": int(rows[-1]["ymd"]),
        "o": float(rows[0]["o"]),
        "h": max(float(row["h"]) for row in rows),
        "l": min(float(row["l"]) for row in rows),
        "c": float(rows[-1]["c"]),
        "v": sum(float(row.get("v") or 0.0) for row in rows),
        "daily_bar_count": len(rows),
    }


def _forward_return_by_code_date(by_code: dict[str, list[dict[str, Any]]]) -> dict[tuple[str, int], float]:
    out: dict[tuple[str, int], float] = {}
    for code, bars in by_code.items():
        for index, row in enumerate(bars[:-20]):
            close = float(row["c"])
            if close <= 0:
                continue
            out[(code, int(row["ymd"]))] = float(bars[index + 20]["c"]) / close - 1.0
    return out


def _build_weekly_two_ma_candidates(
    ...
    *,
    ...
    forward = _forward_return_by_code_date(by_code)
    events: list[dict[str, Any]] = []
    for code, daily_bars in by_code.items():
        weeks = _weekly_bars(daily_bars)
        for index, week in enumerate(weeks):
            ...
            if ymd < start_dt or ymd > end_dt or ymd not in buy_dates or code in buy_codes_by_date[ymd]:
                continue
            forward_return_20 = forward.get((code, ymd))
            if forward_return_20 is None:
                continue
            features = _two_ma_features(weeks[index - 25 : index + 1])
            ...
            events.append(
                {
                    ...
                    "setup_type": "two_ma_simultaneous_breakout_weekly_long_oos_v1",
                    "forward_return_20": forward_return_20,
                    "scene_visual_key": f"weekly_{PATTERN_LABEL}",
                    "market_scene": "weekly_low_stagnation_reversal",
                    "trade_side": "long",
                    "action_bias": "buy_weekly_two_ma_simultaneous_breakout",
                    "shape_intent": f"weekly_{PATTERN_LABEL}",
                    "entry_timing": "weekly_close_break_above_ma5_and_ma20",
                    "in_existing_buy_pool": False,
                    "weekly_signal_date": ymd,
...
def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-two_ma_simultaneous_breakout_weekly_long_oos_v1"
    output_dir.mkdir(parents=True, exist_ok=True)
    con = duckdb.connect(str(db_path), read_only=True)
    try:
        buy_rows = _load_side_signal_rows(con, start_dt=start_dt, end_dt=end_dt, side="buy")
        by_code = _load_daily(con, start_dt=start_dt, end_dt=end_dt, history=420, forward=20)
    finally:
        con.close()
    ...
    candidates = _build_weekly_two_ma_candidates(by_code, buy_codes_by_date=buy_codes_by_date, start_dt=start_dt, end_dt=end_dt)
    ...
    coverage = {
        ...
        "pattern_label": f"weekly_{PATTERN_LABEL}",
    }
    decision = _oos_decision(oos_compare, monthly, coverage)
    result = {
        "schema_version": "tradex_two_ma_simultaneous_breakout_weekly_long_oos_v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "authoritative_result": True,
        "research_phase": "effectiveness_judgment",
        "fixed_evaluation_conditions": {
            "source_table": "signal_decision_daily + daily_bars weekly resample outside-gap scan",
            "start_dt": start_dt,
            "end_dt": end_dt,
            ...
            "changed_axis": f"weekly_{PATTERN_LABEL}",
            "selection_rule": "weekly resample of trader two-MA simultaneous breakout; candidate only when weekly signal date is also a buy-candidate date; at most one outside-gap candidate per date",
        ...
        "observed_branching": {
            ...
            "selection_divergence_reason": "historical_oos_outside_gap_weekly_two_ma_simultaneous_breakout_candidate_added_to_buy_pool",
        },
        ...
        "remaining_risks": [
            "weekly signal is mapped to the week-ending daily candidate date only",
            ...
            "weekly resample uses available daily bars and not a separate weekly source",
        ],
    }
