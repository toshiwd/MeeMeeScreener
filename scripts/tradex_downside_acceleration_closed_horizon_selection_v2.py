...
AXIS_ID = "tradex_downside_acceleration_closed_horizon_selection_v2"
SCHEMA_PREFIX = "tradex_downside_acceleration_closed_horizon_selection_v2"
DEFAULT_OUTPUT_DIR_NAME = "downside_acceleration_closed_horizon_selection_v2"
DEFAULT_TOP_K = 5
DEFAULT_MIN_TRAIN_MONTHS = 6
DEFAULT_LOOKBACK_MONTHS = 12

REQUIRED_ARTIFACTS = (
    "downside_acceleration_closed_horizon_contract.json",
    "downside_acceleration_closed_horizon_bucket_model.json",
    "downside_acceleration_closed_horizon_monthly_rankings.csv",
    "downside_acceleration_closed_horizon_compare.json",
    "downside_acceleration_closed_horizon_yearly_performance.json",
    "downside_acceleration_closed_horizon_decision.json",
...
def _row_label(row: dict[str, Any]) -> int:
    ...
    if ret20 is None:
        raise ValueError("short_ret_20 is required for closed-horizon labeling")
...
def _acceleration_target(row: dict[str, Any]) -> float:
    ...
    if ret5 is None or ret10 is None or ret20 is None:
        raise ValueError("short_ret_5, short_ret_10, and short_ret_20 are required for closed-horizon target")
...
def run_pipeline(*, db_path: str | Path | None = None, output_dir: str | Path | None = None, start_ymd: int = 20240101, end_ymd: int = 20260515, top_k: int = DEFAULT_TOP_K, min_train_months: int = DEFAULT_MIN_TRAIN_MONTHS, lookback_months: int = DEFAULT_LOOKBACK_MONTHS) -> dict[str, Any]:
    ...
    output_root = Path(output_dir).expanduser().resolve() if output_dir else Path(r"G:\Tradex\downside_acceleration_closed_horizon_selection_v2")
    ...
    contract = {
        ...
        "objective": "closed-horizon statistical downside-acceleration short selector",
        ...
        "selection_policy": {
            ...
            "label": "known short_ret_20 > 0",
            "label_definition": "good short if known 20d forward short return is positive; unknown horizons are excluded, not counted as losses",
            "target_definition": "0.4 * known short_ret_5 + 0.3 * known short_ret_10 + 0.3 * known short_ret_20",
        },
    ...
    _write_json(output_root / "downside_acceleration_closed_horizon_contract.json", contract)
    _write_json(output_root / "downside_acceleration_closed_horizon_bucket_model.json", bucket_model)
    _write_csv(output_root / "downside_acceleration_closed_horizon_monthly_rankings.csv", pd.DataFrame(monthly_rankings))
    _write_json(output_root / "downside_acceleration_closed_horizon_compare.json", compare_json)
    _write_json(output_root / "downside_acceleration_closed_horizon_yearly_performance.json", {"schema_version": f"{SCHEMA_PREFIX}_yearly_performance_v1", "axis_id": AXIS_ID, "yearly": yearly_summary.to_dict(orient="records")})
    _write_json(output_root / "downside_acceleration_closed_horizon_decision.json", decision_payload)
