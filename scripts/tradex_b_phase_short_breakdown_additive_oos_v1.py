...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/b_phase_short_breakdown_additive_oos_v1")
REQUIRED_SHAPE_INTENT = "b_phase_breakdown_down"
BASE_SIGNAL_KEY = "sideways_b_phase|sell_breakdown_or_rejection|short"
...
def _build_b_phase_short_candidates(
    ...
    *,
    ...
    for code, bars in by_code.items():
        for index, bar in enumerate(bars):
            ...
            prior_range20_low = min(lows[-21:-1])
            range20_pct = (range20_high / range20_low - 1.0) if range20_low else 99.0
            latest_20_return = (close / closes[-21] - 1.0) if closes[-21] else 99.0
            ma5 = mean(closes[-5:])
            if not (range20_pct <= 0.08 and abs(latest_20_return) <= 0.05 and close < prior_range20_low * 0.995 and close < ma5):
                continue
            ...
            events.append(
                {
                    ...
                    "setup_type": "b_phase_short_breakdown_additive_oos_v1",
                    "forward_return_20": -(float(bars[index + 20]["c"]) / close - 1.0),
                    "scene_signal_key": BASE_SIGNAL_KEY,
...
def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-b_phase_short_breakdown_additive_oos_v1"
    ...
    candidates = _build_b_phase_short_candidates(by_code, sell_codes_by_date=sell_codes_by_date, start_dt=start_dt, end_dt=end_dt)
    ...
    result = {
        "schema_version": "tradex_b_phase_short_breakdown_additive_oos_v1",
        ...
        "fixed_evaluation_conditions": {
            ...
            "changed_axis": "b_phase_short_breakdown_outside_gap_candidate",
            "selection_rule": "shape_intent == b_phase_breakdown_down, at most one outside-gap candidate per sell-candidate date ranked by lower ma20_slope_10",
        },
        "scope": {
            "tradex_only": True,
        ...
        "observed_branching": {
            ...
            "selection_divergence_reason": "historical_oos_outside_gap_b_phase_short_breakdown_candidate_added_to_sell_pool",
        },
        ...
        "remaining_risks": [
            "B phase probe was sparse in the in-sample window",
