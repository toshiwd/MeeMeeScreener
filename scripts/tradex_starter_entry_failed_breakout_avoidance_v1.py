...
AXIS_ID = "starter_entry_failed_breakout_avoidance_v1"
...
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\starter_entry_failed_breakout_avoidance_v1")
TOPK_VALUES = (5, 10, 20)
READ_COLUMNS = [
    "decision_date",
    "code",
...
REQUIRED_ARTIFACTS = (
    "failed_breakout_summary.json",
    "failed_breakout_rows.csv",
    "bucket_metrics.json",
...
def load_rows(input_root: Path) -> pd.DataFrame:
    ...
    for col in ["decision_date", "year", "baseline_rank", "baseline_score", "upper_wick_ratio", "volume_ma20_ratio", "ret20"]:
        rows[col] = pd.to_numeric(rows[col], errors="coerce")
    for col in ["failed_high_update", "large_bearish_candle", "large_bullish_candle", "monthly_high_zone_proxy", "monthly_box_breakout_proxy", "monthly_box_inside_proxy", "path20_available", "starter_good", "starter_bad", "selected_loser", "immediate_adverse_entry"]:
...
def score_axis(rows: pd.DataFrame) -> pd.DataFrame:
    out = rows.copy()
    breakout_zone = out["monthly_high_zone_proxy"] | out["monthly_box_breakout_proxy"]
    failed_breakout_risk = breakout_zone & (out["failed_high_update"] | out["large_bearish_candle"] | ((out["upper_wick_ratio"] >= 0.3) & (out["volume_ma20_ratio"] >= 1.1)))
    healthy_breakout = breakout_zone & out["large_bullish_candle"] & (out["upper_wick_ratio"] <= 0.15) & (out["volume_ma20_ratio"] >= 1.0)
    out["failed_breakout_action"] = "retain"
    out.loc[failed_breakout_risk, "failed_breakout_action"] = "demote_failed_breakout_high_reject"
    out.loc[healthy_breakout, "failed_breakout_action"] = "retain_healthy_breakout"
    delta = pd.Series(0.0, index=out.index).mask(failed_breakout_risk, -40.0).mask(healthy_breakout, 2.0)
    out["failed_breakout_sort_score"] = out["baseline_score"].fillna(0.0) + delta - out["baseline_rank"].fillna(9999) * 0.001
    out["failed_breakout_rank"] = (
        out.sort_values(["decision_date", "failed_breakout_sort_score", "baseline_rank", "code"], ascending=[True, False, True, True])
...
def summarize(frame: pd.DataFrame, rank_col: str, topk: int) -> dict[str, Any]:
    ...
    return {"n": int(len(g)), "mean_ret20": _mean(g, "ret20"), "bad_pick_rate": _rate(g["starter_bad"]) if not g.empty else None, "severe_loss_rate": _rate(g["ret20"] <= -0.05) if not g.empty else None, "selected_loser_rate": _rate(g["selected_loser"]) if not g.empty else None}
...
def topk_comparison(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for period, pr in periods(rows):
        for topk in TOPK_VALUES:
            ...
            v = summarize(pr, "failed_breakout_rank", topk)
            rec.append({"period": period, "topk": topk, **{f"baseline_{k}": x for k, x in b.items()}, **{f"challenger_{k}": x for k, x in v.items()}, "delta_mean_ret20": None if b["mean_ret20"] is None or v["mean_ret20"] is None else v["mean_ret20"] - b["mean_ret20"], "delta_bad_pick_rate": None if b["bad_pick_rate"] is None or v["bad_pick_rate"] is None else v["bad_pick_rate"] - b["bad_pick_rate"], "delta_severe_loss_rate": None if b["severe_loss_rate"] is None or v["severe_loss_rate"] is None else v["severe_loss_rate"] - b["severe_loss_rate"]})
    ...
    for period, pr in periods(rows):
        for topk in TOPK_VALUES:
            ...
            for _, g in pr.groupby("decision_date", sort=True):
                ...
                var = set(g[g["failed_breakout_rank"] <= topk]["code"].astype(str))
...
def boundary_metrics(rows: pd.DataFrame) -> dict[str, Any]:
    ...
    out["changed_rank_count"] = int((rows["baseline_rank"] != rows["failed_breakout_rank"]).sum())
    out["selection_divergence_reason"] = "fixed point-in-time failed breakout/high rejection demotion"
    return out
...
def decide(comp: pd.DataFrame, repl: dict[str, Any], boundary: dict[str, Any]) -> dict[str, Any]:
    ...
    if boundary["changed_top10_members_count"] < 10:
        ...
        decision, reason = "close_branch_no_reusable_signal", "failed_breakout_avoidance_has_no_clear_same_condition_edge"
    return {"axis_id": AXIS_ID, "research_decision": decision, "reason_typed": [reason], "replacement_delta_ret20_top10_recent": r10["replacement_delta_ret20"], "meemee_reflectable_candidate": False, "runtime_db_write": False, "production_ranking_changed": False, "publish_allowed": False, "validated_buy_count": 0, "active_gate_created": False}


def run(input_root: Path, output_root: Path) -> Path:
    out = output_root / f"{_now_tag()}-starter-entry-failed-breakout-avoidance-v1"
    ...
    _write_json(out / "failed_breakout_summary.json", {"axis_id": AXIS_ID, "input_rows": int(len(rows)), **boundary})
    rows.to_csv(out / "failed_breakout_rows.csv", index=False)
    _write_json(out / "bucket_metrics.json", {"failed_breakout_action": rows.groupby("failed_breakout_action").size().to_dict()})
    ...
    _write_json(out / "source_coverage.json", {"row_count": int(len(rows)), "date_count": int(rows["decision_date"].nunique()), "confirmed_bars_only": True, "research_fallback_used": False, "feature_coverage": {c: float(rows[c].notna().mean()) for c in READ_COLUMNS if c in rows and c not in {"ret20", "starter_good", "starter_bad", "selected_loser", "immediate_adverse_entry"}}})
    _write_json(out / "lineage.json", {"input_family_source_root": input_root, "closed_prior_axis": r"G:\Tradex\starter_entry_daily_timing_quality_v1\20260525T074533Z-starter-entry-daily-timing-quality-v1"})
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="TRADEX starter-entry failed breakout avoidance pretest")
