...
AXIS_ID = "tradex_downside_prebreak_event_selection_v3"
SCHEMA_PREFIX = "tradex_downside_prebreak_event_selection_v3"
DEFAULT_OUTPUT_DIR_NAME = "downside_prebreak_event_selection_v3"
DEFAULT_TOP_K = 5
DEFAULT_MIN_TRAIN_MONTHS = 6
DEFAULT_LOOKBACK_MONTHS = 12

REQUIRED_ARTIFACTS = (
    "downside_prebreak_event_contract.json",
    "downside_prebreak_event_bucket_model.json",
    "downside_prebreak_event_monthly_rankings.csv",
    "downside_prebreak_event_compare.json",
    "downside_prebreak_event_yearly_performance.json",
    "downside_prebreak_event_decision.json",
...
def run_pipeline(*, db_path: str | Path | None = None, output_dir: str | Path | None = None, start_ymd: int = 20240101, end_ymd: int = 20260515, top_k: int = DEFAULT_TOP_K, min_train_months: int = DEFAULT_MIN_TRAIN_MONTHS, lookback_months: int = DEFAULT_LOOKBACK_MONTHS) -> dict[str, Any]:
    ...
    output_root = Path(output_dir).expanduser().resolve() if output_dir else Path(r"G:\Tradex\downside_prebreak_event_selection_v3")
    ...
    candidate_rows = [row for row in rows if _is_closed_horizon(row)]
    by_month: dict[int, list[dict[str, Any]]] = defaultdict(list)
    ...
    for month in sorted(by_month):
        ...
        train_rows = [row for m in train_months for row in by_month[m] if _is_closed_horizon(row)]
        month_rows = [dict(row) for row in by_month[month] if _is_closed_horizon(row)]
    ...
    contract = {
        ...
        "objective": "near-term downside pre-break event short selector",
        "selection_universe": "closed-horizon short candidates from entry_precision_short_audit rows",
        "selection_policy": {
            "walk_forward": True,
            "top_k": int(top_k),
    ...
    _write_json(output_root / "downside_prebreak_event_contract.json", contract)
    _write_json(output_root / "downside_prebreak_event_bucket_model.json", bucket_model)
    _write_csv(output_root / "downside_prebreak_event_monthly_rankings.csv", pd.DataFrame(monthly_rankings))
    _write_json(output_root / "downside_prebreak_event_compare.json", compare_json)
    _write_json(output_root / "downside_prebreak_event_yearly_performance.json", {"schema_version": f"{SCHEMA_PREFIX}_yearly_performance_v1", "axis_id": AXIS_ID, "yearly": yearly_summary.to_dict(orient="records")})
    _write_json(output_root / "downside_prebreak_event_decision.json", decision_payload)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Statistical downside pre-break event short selector.")
