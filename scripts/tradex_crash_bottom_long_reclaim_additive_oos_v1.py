...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/crash_bottom_long_reclaim_additive_oos_v1")
REQUIRED_SHAPE_INTENT = "crash_bottom_confirmed_above_20ma"
BASE_SIGNAL_KEY = "crash_bottoming_phase|probe_bottom_only_after_ma_reclaim|long_probe_or_short_cover|keep_probe_candidate"
...
def _select_one_per_date(events: list[dict[str, Any]]) -> dict[int, dict[str, Any]]:
    ...
    for event in events:
        ...
        if current is None:
            ...
            continue
        current_rank = (
            -float(current.get("recent_downtrend_drop_pct_abs") or 0.0),
            -float(current.get("ma20_slope_10") or -9.0),
            str(current["code"]),
        )
        event_rank = (
            -float(event.get("recent_downtrend_drop_pct_abs") or 0.0),
...
def _cheap_crash_bottom_reclaim_prefilter(window: list[dict[str, Any]]) -> bool:
    if len(window) < 60:
        return False
    closes = [float(row["c"]) for row in window]
    highs = [float(row["h"]) for row in window]
    close = closes[-1]
    recent_drop = close / max(highs[-60:]) - 1.0 if highs[-60:] else 0.0
    current_ma20 = mean(closes[-20:])
    previous_ma20 = mean(closes[-21:-1])
    consecutive_above20_at_least_two = closes[-1] > current_ma20 and closes[-2] > previous_ma20
    return bool(recent_drop <= -0.08 and consecutive_above20_at_least_two)


def _build_crash_bottom_long_candidates(
    ...
    *,
    ...
    for code, bars in by_code.items():
        for index, bar in enumerate(bars):
            ...
            if not _cheap_crash_bottom_reclaim_prefilter(window):
                continue
            ...
            if visual.get("decision") != "keep_probe_candidate":
                continue
            recent_downtrend_drop_pct = shape.get("metrics", {}).get("recent_downtrend_drop_pct")
            events.append(
                {
                    "dt": ymd,
                    "code": code,
                    ...
                    "setup_type": "crash_bottom_long_reclaim_additive_oos_v1",
                    ...
                    "recent_downtrend_drop_pct": recent_downtrend_drop_pct,
                    "recent_downtrend_drop_pct_abs": abs(float(recent_downtrend_drop_pct or 0.0)),
...
def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-crash_bottom_long_reclaim_additive_oos_v1"
    ...
    candidates = _build_crash_bottom_long_candidates(by_code, buy_codes_by_date=buy_codes_by_date, start_dt=start_dt, end_dt=end_dt)
    ...
    result = {
        "schema_version": "tradex_crash_bottom_long_reclaim_additive_oos_v1",
        ...
        "fixed_evaluation_conditions": {
            ...
            "changed_axis": "crash_bottom_confirmed_20ma_reclaim_keep_visual_outside_gap_candidate",
            "selection_rule": "shape_intent == crash_bottom_confirmed_above_20ma and visual_decision == keep_probe_candidate; at most one outside-gap candidate per buy-candidate date ranked by deeper recent drop then higher ma20_slope_10",
        ...
        "observed_branching": {
            ...
            "selection_divergence_reason": "historical_oos_outside_gap_crash_bottom_long_reclaim_candidate_added_to_buy_pool",
        },
