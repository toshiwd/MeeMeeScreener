...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/crash_bottoming_signal_probe_v1")
...
def _load_daily(con: duckdb.DuckDBPyConnection, *, start_dt: int, end_dt: int, history: int, forward: int) -> dict[str, list[dict[str, Any]]]:
    rows = con.execute(
        """
        WITH normalized AS (
            SELECT
    ...
    return {code: bars for code, bars in by_code.items() if len(bars) >= history + forward + 1}


def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-crash_bottoming_signal_probe_v1"
    ...
    for code, bars in by_code.items():
        for index in range(160, len(bars) - 20):
            ...
            if not shape.get("is_crash_bottoming_setup"):
                continue
            events.append(
                {
                    "dt": ymd,
    ...
    for event in events:
        labels[str(event["shape_intent"])].append(float(event["forward_return_20"]))
        timing[str(event["entry_timing"])].append(float(event["forward_return_20"]))
        same_date = universe_by_date.get(int(event["dt"]), [])
        if same_date:
            baseline_values.append(mean(same_date))

    event_values = [float(event["forward_return_20"]) for event in events]
    event_metrics = _metrics(event_values)
    baseline_metrics = _metrics(baseline_values)
    delta_mean = None
    if event_metrics["mean_ret20"] is not None and baseline_metrics["mean_ret20"] is not None:
        delta_mean = float(event_metrics["mean_ret20"]) - float(baseline_metrics["mean_ret20"])
    if len(events) < 30:
        decision = {"judgment": "hold", "reason_type": "insufficient_sample"}
    elif (delta_mean or 0.0) > 0.01 and (event_metrics["bad_loser_rate_20"] or 1.0) <= (baseline_metrics["bad_loser_rate_20"] or 0.0) + 0.02:
        decision = {"judgment": "keep", "reason_type": "positive_ret20_vs_same_date_baseline"}
    else:
        decision = {"judgment": "drop", "reason_type": "no_positive_same_date_edge"}

    result = {
        "schema_version": "tradex_crash_bottoming_signal_probe_v1",
        ...
        "compare": {
            ...
            "same_date_baseline": baseline_metrics,
            "delta": {"mean_ret20": _round(delta_mean)},
            "by_shape_intent": {key: _metrics(values) for key, values in sorted(labels.items())},
            "by_entry_timing": {key: _metrics(values) for key, values in sorted(timing.items())},
        },
        "authoritative_rollup_decision": decision["judgment"],
        "reason_type": decision["reason_type"],
        "meemee_reflectable": False,
        "remaining_risks": [
            "candidate-generation style event study, not topK ranking replay",
            "same-date baseline is aggregate universe mean, not matched liquidity/rank control",
            "single-period in-sample probe",
        ],
    }
    compare_path = output_dir / "crash_bottoming_signal_probe_compare.json"
    decision_path = output_dir / "crash_bottoming_signal_probe_decision.json"
    ledger_path = output_dir / "crash_bottoming_signal_probe_ledger.jsonl"
    result["artifacts"] = {
...
def main() -> None:
    ...
    print(json.dumps({"decision": result["authoritative_rollup_decision"], "reason_type": result["reason_type"], "delta": result["compare"]["delta"], "events": result["compare"]["events"], "baseline": result["compare"]["same_date_baseline"]}, ensure_ascii=False, indent=2))
