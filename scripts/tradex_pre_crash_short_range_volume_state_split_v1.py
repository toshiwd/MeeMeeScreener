...
AXIS_ID = "pre_crash_short_range_volume_state_split_v1"
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/pre_crash_short_range_volume_state_split_v1")
TOP_K = 3
COOLDOWN_DAYS = 5
PROFIT_TARGET = 0.20
STOP_LOSS = 0.08
...
def _thresholds(df: pd.DataFrame, feature: str) -> list[float]:
    ...
    return sorted({float(values.quantile(q)) for q in (0.33, 0.50, 0.67, 0.75)}) if not values.empty else []


def _evaluate(selected: pd.DataFrame) -> tuple[list[dict[str, Any]], dict[str, Any] | None]:
    baseline = _summarize(selected)
    rows: list[dict[str, Any]] = []
    for range_threshold in _thresholds(selected, "range_40_20"):
        for vol_threshold in _thresholds(selected, "last_vol_ratio"):
            mask = (selected["range_40_20"].astype(float) >= range_threshold) & (
                selected["last_vol_ratio"].astype(float) <= vol_threshold
            )
            entry_ready = selected[mask].copy()
            avoid = selected[~mask].copy()
            er = _summarize(entry_ready)
            av = _summarize(avoid)
            if er["n"] < MIN_ENTRY_N or er["months"] < MIN_MONTHS:
                continue
            ...
            positive_month_lift = float(er["positive_month_rate"] - baseline["positive_month_rate"])
            avoid_mean_delta = float(av["mean_short_ret"] - baseline["mean_short_ret"])
            row = {
                "axis_id": f"range40_ge_{range_threshold:.6f}__vol_le_{vol_threshold:.6f}",
                "range_40_20_min": range_threshold,
                "last_vol_ratio_max": vol_threshold,
                "baseline": baseline,
                ...
                "entry_ready_positive_month_lift": positive_month_lift,
                "avoid_mean_delta": avoid_mean_delta,
                "decision": (
                    "keep"
                    if mean_lift >= 0.006
                    and target_lift >= 0.025
                    and stop_delta <= 0.0
                    and positive_month_lift >= 0.0
...
def _examples(selected: pd.DataFrame, best: dict[str, Any] | None) -> list[dict[str, Any]]:
    ...
    mask = (selected["range_40_20"].astype(float) >= float(best["range_40_20_min"])) & (
        selected["last_vol_ratio"].astype(float) <= float(best["last_vol_ratio_max"])
    )
...
def run(db_path: Path, output_root: Path, code_limit: int | None) -> Path:
    ...
    decision = {
        "authoritative_decision": "keep_range_volume_state_split" if keep else "drop_no_range_volume_state_split_edge",
        "candidate_local_decision": best,
        "promotion_allowed": False,
        "meemee_reflection": False,
        "production_ranking_change": False,
        "runtime_db_write": False,
        "reason": "single interaction-axis range40 plus low volume EntryReady/Avoid split on pt20_sl8; costs and borrow ignored by user request",
    }
    contract = {
        "axis_id": AXIS_ID,
        ...
        "fixed_evaluation_conditions": {
            ...
            "changed_axis": "range40/low-volume interaction only",
    ...
    _write_json(run_dir / "range_volume_state_leaderboard.json", {"rows": leaderboard})
    _write_jsonl(run_dir / "range_volume_state_examples.jsonl", _examples(selected, best) if not selected.empty else [])
    ...
    _write_json(
        run_dir / "_ARTIFACT_COMPLETE.json",
        {
            ...
            "required_files": [
                ...
                "run_manifest.json",
                "range_volume_state_leaderboard.json",
                "range_volume_state_examples.jsonl",
