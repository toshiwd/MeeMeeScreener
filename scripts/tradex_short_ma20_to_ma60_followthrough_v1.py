...
AXIS_ID = "short_ma20_to_ma60_followthrough_v1"
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\short_ma20_to_ma60_followthrough_v1")
...
base AS (
  SELECT
    *,
    ...
    min(l) OVER (PARTITION BY code ORDER BY ymd ROWS BETWEEN 19 PRECEDING AND CURRENT ROW) AS low20,
...
events AS (
  SELECT
    *,
    ...
    ma20 / NULLIF(ma60, 0) - 1 AS ma20_vs_ma60,
...
def _exit_metrics(row: dict[str, Any], horizon: int) -> dict[str, Any]:
    ...
    for day in range(1, horizon + 1):
        ...
        if target_day is None and ma60 is not None and float(low) <= float(ma60):
            target_day = day
            target_ret = (entry - float(ma60)) / entry
        if stop_day is None and ma20 is not None and float(close) >= float(ma20):
            stop_day = day
    first_event = "time"
    first_day = horizon
    if target_day is not None and (stop_day is None or target_day <= stop_day):
        first_event = "target_ma60"
        first_day = target_day
    elif stop_day is not None:
        first_event = "ma20_reclaim_stop"
        first_day = stop_day
    return {
        "target_hit_10d": target_day is not None,
        "target_day": target_day,
        "ma20_reclaim_stop_10d": stop_day is not None,
        "stop_day": stop_day,
...
def _classify(row: dict[str, Any]) -> list[str]:
    tags = ["baseline_ma20_break_to_ma60"]
    close_pos = row.get("close_pos")
    range60_pos = row.get("range60_pos")
    ret60 = row.get("ret60")
    ...
    if range60_pos is not None and float(range60_pos) >= 0.70:
        tags.append("high_zone")
    if ret60 is not None and float(ret60) >= 0.20:
    ...
    if "weak_close" in tags and "high_zone" in tags:
        tags.append("weak_close_high_zone")
    if "weak_close" in tags and "post_60d_rise" in tags:
        tags.append("weak_close_after_rise")
    if "high_zone" in tags and "upper_wick_pressure" in tags:
...
def _summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    stop_first_count = sum(1 for row in rows if row["first_event"] == "ma20_reclaim_stop")
    target_first_count = sum(1 for row in rows if row["first_event"] == "target_ma60")
    return {
        ...
        "ma20_reclaim_stop_first_rate": _round(stop_first_count / len(rows)),
...
def _decision(summary: dict[str, Any], baseline: dict[str, Any]) -> tuple[str, str]:
    ...
    if hit >= 0.58 and first >= 0.45 and mean_ret > 0 and positive_month >= 0.55 and hit >= baseline_hit + 0.04:
        return "keep", "target_hit_and_month_breadth_improved"
...
def run(*, db_path: Path, output_root: Path, start_ymd: int, end_ymd: int | None, limit_events: int | None) -> Path:
    ...
    for row in rows:
        ...
        events.append({
            ...
            "dist_ma60": _round(float(row["dist_ma60"])),
    ...
    baseline = _summary(tag_rows.get("baseline_ma20_break_to_ma60", []))
    leaderboard = []
    ...
    authoritative_reason = (
        "at_least_one_filter_improved_target_hit_under_fixed_conditions"
        if keep
    ...
    compare = {
        ...
        "fixed_evaluation_conditions": {
            ...
            "event": "prev close >= prev 20MA and current close < current 20MA while current close > current 60MA",
            "entry": "signal day close",
            "target": "daily low touches same-day 60MA within next 10 trading bars",
            "stop_observation": "daily close reclaims same-day 20MA within next 10 trading bars",
