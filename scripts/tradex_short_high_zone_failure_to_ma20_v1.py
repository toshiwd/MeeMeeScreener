...
AXIS_ID = "short_high_zone_failure_to_ma20_v1"
DEFAULT_OUTPUT_ROOT = Path(r"G:\Tradex\short_high_zone_failure_to_ma20_v1")
...
base AS (
  SELECT
    *,
    ...
    lag(h, 1) OVER w AS prev_h,
    lag(l, 1) OVER w AS prev_l,
    lag(c, 20) OVER w AS c_lag20,
    lag(c, 60) OVER w AS c_lag60,
    max(h) OVER (PARTITION BY code ORDER BY ymd ROWS BETWEEN 19 PRECEDING AND CURRENT ROW) AS high20,
...
events AS (
  SELECT
    *,
    ...
    c / NULLIF(ma20, 0) - 1 AS dist_ma20,
    ...
    h / NULLIF(high20, 0) AS high_vs_high20,
    h / NULLIF(high60, 0) AS high_vs_high60,
    ma20 / NULLIF(ma20_lag5, 0) - 1 AS ma20_slope5
  FROM projected
  WHERE ma20 IS NOT NULL
    ...
    AND c > ma20
    AND c > ma60
    AND (c - low60) / NULLIF(high60 - low60, 0) >= 0.70
    AND h / NULLIF(high60, 0) >= 0.95
)
SELECT *
FROM events
ORDER BY ymd, code
...
def _exit_metrics(row: dict[str, Any], horizon: int) -> dict[str, Any]:
    ...
    for day in range(1, horizon + 1):
        ...
        if target_day is None and ma20 is not None and float(low) <= float(ma20):
            target_day = day
            target_ret = (entry - float(ma20)) / entry
        if stop_day is None and float(close) >= event_high:
            stop_day = day
    first_event = "time"
    first_day = horizon
    if target_day is not None and (stop_day is None or target_day <= stop_day):
        first_event = "target_ma20"
        first_day = target_day
    elif stop_day is not None:
        first_event = "event_high_reclaim_stop"
        first_day = stop_day
    return {
        "target_hit_10d": target_day is not None,
        "target_day": target_day,
        "event_high_reclaim_stop_10d": stop_day is not None,
        "stop_day": stop_day,
...
def _classify(row: dict[str, Any]) -> list[str]:
    tags = ["baseline_high_zone_failure_to_ma20"]
    close_pos = row.get("close_pos")
    upper_wick = row.get("upper_wick_ratio")
    body_ratio = row.get("body_ratio")
    volume_vs20 = row.get("volume_vs20")
    ret20 = row.get("ret20")
    high_vs_high60 = row.get("high_vs_high60")
    ma20_slope5 = row.get("ma20_slope5")
    bearish = float(row["c"]) < float(row["o"])
    if upper_wick is not None and float(upper_wick) >= 0.35:
        tags.append("upper_wick_pressure")
    if close_pos is not None and float(close_pos) <= 0.45:
        tags.append("weak_close")
    if bearish and body_ratio is not None and float(body_ratio) >= 0.45:
        tags.append("bearish_body")
    if float(row["c"]) < float(row["ma7"]):
        tags.append("below_ma7")
    if volume_vs20 is not None and float(volume_vs20) >= 1.25:
        tags.append("volume_confirmed")
    if ret20 is not None and float(ret20) >= 0.15:
        tags.append("post_20d_rise")
    if high_vs_high60 is not None and float(high_vs_high60) >= 0.985:
        tags.append("near_60d_high")
    if ma20_slope5 is not None and float(ma20_slope5) <= 0:
        tags.append("ma20_flat_or_down")
    if "upper_wick_pressure" in tags and "weak_close" in tags:
        tags.append("upper_wick_weak_close")
    if "below_ma7" in tags and "weak_close" in tags:
        tags.append("below_ma7_weak_close")
    if "near_60d_high" in tags and "upper_wick_pressure" in tags:
        tags.append("near_high_upper_wick")
    return tags
...
def _summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    target_first_count = sum(1 for row in rows if row["first_event"] == "target_ma20")
    stop_first_count = sum(1 for row in rows if row["first_event"] == "event_high_reclaim_stop")
    return {
        ...
        "event_high_reclaim_stop_first_rate": _round(stop_first_count / len(rows)),
...
def _decision(summary: dict[str, Any], baseline: dict[str, Any]) -> tuple[str, str]:
    ...
    stop_first = float(summary.get("event_high_reclaim_stop_first_rate") or 0.0)
    ...
    if hit >= 0.58 and first >= 0.48 and stop_first <= 0.35 and mean_ret > 0 and positive_month >= 0.55 and hit >= baseline_hit + 0.03:
        return "keep", "target_hit_and_stop_profile_improved"
...
def run(*, db_path: Path, output_root: Path, start_ymd: int, end_ymd: int | None, limit_events: int | None) -> Path:
    ...
    for row in rows:
        ...
        events.append({
            ...
            "event_high": _round(float(row["h"])),
            "ma7": _round(float(row["ma7"])),
            "ma20": _round(float(row["ma20"])),
            "ma60": _round(float(row["ma60"])),
            "dist_ma20": _round(float(row["dist_ma20"])),
            "range60_pos": _round(float(row["range60_pos"])) if row.get("range60_pos") is not None else None,
            ...
            "upper_wick_ratio": _round(float(row["upper_wick_ratio"])) if row.get("upper_wick_ratio") is not None else None,
            "close_pos": _round(float(row["close_pos"])) if row.get("close_pos") is not None else None,
            "ma20_slope5": _round(float(row["ma20_slope5"])) if row.get("ma20_slope5") is not None else None,
            "tags": "|".join(tags),
    ...
    baseline = _summary(tag_rows.get("baseline_high_zone_failure_to_ma20", []))
    leaderboard = []
    ...
    compare = {
        ...
        "fixed_evaluation_conditions": {
            ...
            "event": "close above 20MA and 60MA, 60d range position >= 0.70, high >= 95pct of 60d high",
            "entry": "signal day close",
            "target": "daily low touches same-day 20MA within next 10 trading bars",
            "stop_observation": "daily close reclaims signal-day high within next 10 trading bars",
