...
AXIS_ID = "strict_top5_candidate_pool_gate_validation_v1"
SCHEMA_PREFIX = "tradex_strict_top5_candidate_pool_gate_validation"
...
DEFAULT_SOURCE_SUBSET_VALIDATION_ROOT = Path(
    "G:/Tradex/common_top5_candidate_pool_validation_subset_or_label_source_audit_v1/"
    "20260514T233000Z-common-top5-candidate-pool-validation-subset-or-label-source-audit-v1"
)
DEFAULT_OUTPUT_PARENT = Path("G:/Tradex/strict_top5_candidate_pool_gate_validation_v1")
DEFAULT_RUN_ID = "20260514T234500Z-strict-top5-candidate-pool-gate-validation-v1"

REQUIRED_ARTIFACTS = [
    "evaluation_contract.json",
    "run_manifest.json",
    "source_artifact_refs.json",
    "strict_gate_contract.json",
...
def _prepare_frame(rows: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for col in ["ret20_fwd", "baseline_score", "momentum_score", "momentum_rank"]:
    ...
    specs = [
        {
            ...
            "ma5_base_score": None,
            "ma5_membership_weight": 0.0,
            "description": "source baseline score only",
        }
    ]
    weights = [-0.04, -0.02, -0.01, 0.0, 0.01, 0.02, 0.04]
    penalties = [0.0, -0.02, -0.04]
    ma5_bases: list[float | None] = [None, -0.5, 0.0, 0.2]
    for mw in weights:
        for lw in weights:
            for hp in penalties:
                for mb in ma5_bases:
                    if mw == lw == hp == 0.0 and mb is None:
                        continue
                    specs.append(
                        {
                            "variant_id": f"blend_m{mw:+.2f}_l{lw:+.2f}_h{hp:+.2f}_ma5{mb if mb is not None else 'none'}",
                            "momentum_weight": mw,
                            "momentum_low_risk_weight": lw,
                            "momentum_high_risk_penalty": hp,
                            "ma5_base_score": mb,
                            "ma5_membership_weight": 0.01 if mb is not None else 0.0,
                            "description": "research-only point-in-time family/context score adjustment",
                        }
                    )
    return specs
...
def _score(frame: pd.DataFrame, spec: Mapping[str, Any]) -> pd.Series:
    base = frame["baseline_score"].where(frame["baseline_candidate_flag"])
    ma5_base = spec.get("ma5_base_score")
    if ma5_base is not None:
        base = frame["baseline_score"].where(
            frame["baseline_candidate_flag"],
            pd.Series(float(ma5_base), index=frame.index).where(frame["ma5_h12_candidate_flag"]),
        )
    score = base.copy()
    ...
    score = score + frame["ma5_h12_candidate_flag"].astype(float) * float(spec["ma5_membership_weight"])
    return score
...
def _select(frame: pd.DataFrame, score: pd.Series, top_k: int = 5) -> pd.DataFrame:
    ...
    work = work[work["_candidate_score"].notna()].sort_values(
        ["event_date", "_candidate_score", "symbol"], ascending=[True, False, True], kind="stable"
    )
...
def _metrics(selected: pd.DataFrame, universe: pd.DataFrame, date_count: int) -> dict[str, Any]:
    if selected.empty:
        return {
            "candidate_count": 0,
            "top5_avg_ret20": None,
            "top5_big_winner_capture_rate": 0.0,
            "top5_future_top10_capture_rate": 0.0,
            "top5_severe_loss_rate20": 0.0,
            "top5_bad_pick_count": 0,
            "human_selectable_day_rate": 0.0,
            "top5_candidate_diversity": 0.0,
        }
    ...
    return {
        ...
        "top5_big_winner_capture_rate": _rate(int(selected["is_big_winner_ret20_ge_10pct"].sum()), int(universe["is_big_winner_ret20_ge_10pct"].sum())),
        "top5_future_top10_capture_rate": _rate(int(selected["is_future_top10_by_ret20"].sum()), int(universe["is_future_top10_by_ret20"].sum())),
        ...
        "top5_candidate_diversity": float(selected.groupby("event_date")["symbol"].nunique().mean()),
    }
...
def _key_set(frame: pd.DataFrame) -> set[tuple[str, str]]:
    if frame.empty:
        return set()
    return set(zip(frame["event_date"].astype(str), frame["symbol"].astype(str)))
...
def _family_share(selected: pd.DataFrame) -> dict[str, Any]:
    ...
    if not total:
        ...
        ma5_mask = selected["ma5_h12_candidate_flag"].astype(bool)
        momentum_mask = selected["momentum_candidate_flag"].astype(bool) & ~ma5_mask
        baseline_only_mask = selected["baseline_candidate_flag"].astype(bool) & ~selected["momentum_candidate_flag"].astype(bool) & ~ma5_mask
        counts = {
            "baseline_only": int(baseline_only_mask.sum()),
            "momentum_overlap": int(momentum_mask.sum()),
            "ma5_h12": int(ma5_mask.sum()),
        }
    shares = {key: _rate(value, total) for key, value in counts.items()}
    max_family_share = max(shares.values()) if shares else 0.0
    return {"counts": counts, "shares": shares, "max_family_share": max_family_share}
        ...
        rows.append(
            {
                ...
                "top5_severe_loss_rate_delta_vs_baseline": _rate(int(left["severe_loss20"].sum()), len(left)) - _rate(int(right["severe_loss20"].sum()), len(right)),
                "top5_bad_pick_count_delta_vs_baseline": int(left["is_bad_pick"].sum()) - int(right["is_bad_pick"].sum()),
            }
        )
    positive_blocks = sum(1 for row in rows if row["top5_avg_ret20_delta_vs_baseline"] > 0.0)
    return {
        "rows": rows,
        "time_block_count": len(rows),
        "positive_avg_ret20_block_count": positive_blocks,
        "positive_avg_ret20_block_rate": _rate(positive_blocks, len(rows)),
        "effect_remains": len(rows) >= 3 and _rate(positive_blocks, len(rows)) >= 0.60,
    }


def _top3_guardrail(selected: pd.DataFrame, baseline: pd.DataFrame, universe: pd.DataFrame) -> dict[str, Any]:
    sel3 = selected.groupby("event_date", sort=False).head(3).copy()
    base3 = baseline.groupby("event_date", sort=False).head(3).copy()
    oracle3 = (
    ...
    return {
        ...
        "top3_avg_ret20_delta_vs_baseline": float(sel3["ret20_fwd"].mean() - base3["ret20_fwd"].mean()) if not sel3.empty and not base3.empty else None,
        ...
        "top3_severe_loss_rate_delta_vs_baseline": _rate(int(sel3["severe_loss20"].sum()), len(sel3)) - _rate(int(base3["severe_loss20"].sum()), len(base3)),
...
def _gate_row(
    ...
    deltas = {
        ...
        "top5_big_winner_capture_delta_vs_baseline": _delta(metrics["top5_big_winner_capture_rate"], baseline_metrics["top5_big_winner_capture_rate"]),
        "top5_future_top10_capture_delta_vs_baseline": _delta(metrics["top5_future_top10_capture_rate"], baseline_metrics["top5_future_top10_capture_rate"]),
...
def run(args: argparse.Namespace) -> Path:
    ...
    source_repair_decision = _read_json(args.source_field_repair_root / "research_decision.json")
    source_subset_decision = _read_json(args.source_subset_validation_root / "research_decision.json")
    frame = _prepare_frame(source_rows)
    date_count = frame["event_date"].nunique()
    ...
    if pass_rows:
        ...
        authoritative = "strict_top5_candidate_pool_gate_keep_candidate"
        next_axis = "starter_entry_candidate_pretest_v1"
        typed_reasons = ["all_mandatory_top5_candidate_pool_gates_passed"]
    else:
        decision = "drop"
        authoritative = "strict_top5_candidate_pool_gate_failed"
        next_axis = "pattern_family_portfolio_refresh_v2"
        typed_reasons = ["no_variant_satisfied_all_mandatory_top5_candidate_pool_gates"]
        if best:
    ...
    artifacts: dict[str, dict[str, Any]] = {
        "evaluation_contract.json": {
            ...
            "candidate_scoring_scope": "research_only_point_in_time_family_context_adjustments",
        ...
        "run_manifest.json": {
            ...
            "source_subset_validation_root": str(args.source_subset_validation_root),
            "output_root": str(output_root),
            "label_complete_row_count": int(len(frame)),
            "evaluation_date_count": int(date_count),
            "variant_count": len(rows),
        },
        "source_artifact_refs.json": {
            "schema_version": f"{SCHEMA_PREFIX}_source_artifact_refs_v1",
            "source_field_repair_root": str(args.source_field_repair_root),
            "source_field_repair_decision": source_repair_decision.get("authoritative_research_decision"),
            "source_subset_validation_root": str(args.source_subset_validation_root),
            "source_subset_validation_decision": source_subset_decision.get("authoritative_research_decision"),
        },
        "strict_gate_contract.json": {
            "schema_version": f"{SCHEMA_PREFIX}_strict_gate_contract_v1",
            "required_gates": MANDATORY_GATES,
            "gate_pass_policy": "all_mandatory_gates_must_pass",
            "same_condition_policy": "same_label_complete_subset_same_dates_same_top5_top3",
            "time_block_policy": "year_blocks_at_least_60_percent_positive_avg_ret20_delta",
            "family_concentration_policy": "max_selected_family_share_lte_0_90",
            "bad_pick_policy": "absolute_count_must_not_increase",
        },
        "variant_search_space.json": {
            "schema_version": f"{SCHEMA_PREFIX}_variant_search_space_v1",
            "variant_count": len(rows),
            "search_policy": "bounded_research_only_grid",
            "uses_future_labels_in_scoring": False,
            "dimensions": {
                "momentum_weight": [-0.04, -0.02, -0.01, 0.0, 0.01, 0.02, 0.04],
                "momentum_low_risk_weight": [-0.04, -0.02, -0.01, 0.0, 0.01, 0.02, 0.04],
                "momentum_high_risk_penalty": [0.0, -0.02, -0.04],
                "ma5_base_score": [None, -0.5, 0.0, 0.2],
            },
        },
        "strict_gate_leaderboard.json": {
        ...
        "research_decision.json": {
            ...
            "generated_at_utc": generated_at,
            "research_phase": "strict_top5_candidate_pool_gate_validation",
            "boundary": "TRADEX-only",
            "axis_moved": "strict_top5_candidate_pool_gate_validation",
            "source_subset_validation_decision": source_subset_decision.get("authoritative_research_decision"),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-subset-validation-root", type=Path, default=DEFAULT_SOURCE_SUBSET_VALIDATION_ROOT)
