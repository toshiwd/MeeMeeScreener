...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/market_scene_signal_probe_v1")
...
def _load_daily(con: duckdb.DuckDBPyConnection, *, start_dt: int, end_dt: int, history: int, forward: int) -> dict[str, list[dict[str, Any]]]:
    ...
    for code, ymd, open_, high, low, close, volume in rows:
        if open_ is None or high is None or low is None or close is None:
            continue
        by_code[str(code)].append({"ymd": int(ymd), "o": float(open_), "h": float(high), "l": float(low), "c": float(close), "v": float(volume or 0)})
    return {code: bars for code, bars in by_code.items() if len(bars) >= history + forward + 1}


def _judge_groups(groups: dict[str, dict[str, Any]], baseline: dict[str, Any], *, side: str) -> dict[str, Any]:
    keep: list[str] = []
    hold: list[str] = []
    drop: list[str] = []
    ...
    for key, metrics in sorted(groups.items()):
        parts = key.split("|")
        trade_side = parts[2] if len(parts) >= 3 else ""
        side_eligible = (
            (side == "long" and trade_side in {"long", "long_probe_or_short_cover"})
            or (side == "short" and trade_side == "short")
        )
        ...
        if not side_eligible:
            ...
            if mean_delta > 0.0 and bad_delta <= 0.02:
                hold.append(key)
            else:
                drop.append(key)
        elif mean_delta > 0.01 and bad_delta <= 0.02:
            keep.append(key)
        elif mean_delta > 0.0 and bad_delta <= 0.05:
            hold.append(key)
        else:
            drop.append(key)
    if keep:
        judgment = "hold"
        reason_type = f"{side}_has_keep_subscene_requires_followup"
    elif hold:
        judgment = "hold"
        reason_type = f"{side}_weak_or_sparse_subscenes"
    else:
        judgment = "drop"
        reason_type = f"{side}_no_scene_edge"
    return {"judgment": judgment, "reason_type": reason_type, "keep": keep, "hold": hold, "drop": drop}


def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-market_scene_signal_probe_v1"
    ...
    for event in events:
        key = f"{event['market_scene']}|{event['action_bias']}|{event['trade_side']}"
    ...
    result = {
        "schema_version": "tradex_market_scene_signal_probe_v1",
        ...
        "fixed_evaluation_conditions": {
            ...
            "start_dt": start_dt,
            "end_dt": end_dt,
            "signal": "market_scene/action_bias from trade_shape_classifier",
        ...
        "compare": {
            "long_events": _metrics([float(event["long_forward_return_20"]) for event in events]),
            "short_events": _metrics([float(event["short_forward_return_20"]) for event in events]),
            "same_date_long_baseline": baseline_long,
            "same_date_short_baseline": baseline_short,
            "long_by_scene_action_side": long_by_group,
            "short_by_scene_action_side": short_by_group,
        },
        ...
        "reason_type": "scene_subsignals_require_followup" if long_decision["judgment"] == "hold" or short_decision["judgment"] == "hold" else "no_scene_edge",
        ...
        "remaining_risks": [
            ...
            "scene classifier is rule-based and not yet calibrated against screenshots",
        ],
    }
    compare_path = output_dir / "market_scene_signal_probe_compare.json"
    decision_path = output_dir / "market_scene_signal_probe_decision.json"
    ledger_path = output_dir / "market_scene_signal_probe_ledger.jsonl"
    result["artifacts"] = {
