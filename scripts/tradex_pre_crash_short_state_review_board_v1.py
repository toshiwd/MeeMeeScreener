...
AXIS_ID = "pre_crash_short_state_review_board_v1"
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/pre_crash_short_state_review_board_v1")
...
def _review_state(features: dict[str, float | None]) -> tuple[str, list[str]]:
    range40 = features.get("range_40_20")
    vol = features.get("last_vol_ratio")
    if range40 is not None and vol is not None and range40 >= ENTRY_READY_RANGE_40_20_MIN and vol <= ENTRY_READY_LAST_VOL_RATIO_MAX:
        return "EntryReady", [
            f"range_40_20 >= {ENTRY_READY_RANGE_40_20_MIN:.6f}",
            f"last_vol_ratio <= {ENTRY_READY_LAST_VOL_RATIO_MAX:.6f}",
            "kept_by_range_volume_state_split_v1",
        ]
    return "Avoid", [
        "failed_kept_range_volume_entry_ready_rule",
        "historical_complement_had_lower_mean_and_lower_target_hit_rate",
    ]
...
def _candidate_row(code: str, current: pd.Series, features: dict[str, float | None], pattern: str) -> dict[str, Any]:
    ...
    state, state_reasons = _review_state(features)
    target_price = close * (1.0 - PROFIT_TARGET)
    stop_price = close * (1.0 + STOP_LOSS)
    return {
        "code": str(code),
...
def _state_counts(board: list[dict[str, Any]]) -> dict[str, int]:
    counts: dict[str, int] = {"EntryReady": 0, "Watch": 0, "Avoid": 0}
    for row in board:
...
def run(db_path: Path, output_root: Path, code_limit: int | None, recent_sessions: int, limit: int) -> Path:
    ...
    state_counts = _state_counts(board)
    decision = {
        "authoritative_decision": "ready_state_review_board" if board else "ready_empty_state_review_board",
        "candidate_local_decision": {
            ...
            "entry_ready_rule": {
                ...
                "source_axis": "pre_crash_short_range_volume_state_split_v1",
            },
            "profit_take_pct": PROFIT_TARGET,
            "stop_loss_pct": STOP_LOSS,
        },
        ...
        "reason": "review-only state board materialized from kept range-volume split; no validated live short claim",
    }
    contract = {
        "axis_id": AXIS_ID,
        ...
        "fixed_evaluation_conditions": {
            ...
            "profit_take_pct": PROFIT_TARGET,
            "stop_loss_pct": STOP_LOSS,
            "state_rule": "EntryReady if range_40_20>=0.46500567966679285 and last_vol_ratio<=0.9019019159535286 else Avoid",
            "cost_slippage": "ignored_by_user_request",
            "borrow_lending": "ignored_by_user_request",
    ...
    _write_json(run_dir / "state_review_board.json", {"candidates": board})
    _write_jsonl(run_dir / "state_review_board.jsonl", board)
    ...
    _write_json(
        run_dir / "_ARTIFACT_COMPLETE.json",
        {
            ...
            "required_files": [
                ...
                "run_manifest.json",
                "state_review_board.json",
                "state_review_board.jsonl",
