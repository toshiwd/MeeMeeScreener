...
DEFAULT_OUTPUT_ROOT = Path("G:/Tradex/a_phase_anti_long_high_hold_oos_v1")
...
def _anti_long_high_hold_features(window: list[dict[str, Any]]) -> dict[str, Any]:
    rows = window[-240:]
    closes = [float(row["c"]) for row in rows if row.get("c") is not None]
    highs = [float(row["h"]) for row in rows if row.get("h") is not None]
    lows = [float(row["l"]) for row in rows if row.get("l") is not None]
    if len(closes) < 120 or not highs or not lows:
    ...
    ma20 = mean(closes[-20:])
    ma60 = mean(closes) if closes else None
    trend_slope = ma20 / ma60 - 1.0 if ma60 and ma60 > 0 else None
    recent_high = max(highs)
    recent_high_drawdown = close / recent_high - 1.0 if recent_high > 0 else 0.0
    anti_short_high_hold = close_position >= 0.66 and recent_high_drawdown >= -0.12
    return {
        "confirmed": True,
        "display_window_bars": len(rows),
        "close_position_display": _round(close_position),
        "ma20_vs_ma60": _round(trend_slope),
        "recent_high_drawdown": _round(recent_high_drawdown),
        "anti_short_high_hold": anti_short_high_hold,
    }


def _build_window_index(by_code: dict[str, list[dict[str, Any]]]) -> dict[tuple[int, str], list[dict[str, Any]]]:
    out: dict[tuple[int, str], list[dict[str, Any]]] = {}
    for code, bars in by_code.items():
        for index, bar in enumerate(bars):
            if index < 239:
                continue
            out[(int(bar["ymd"]), code)] = bars[index - 239 : index + 1]
    return out


def _apply_anti_long_high_hold(events: list[dict[str, Any]], window_index: dict[tuple[int, str], list[dict[str, Any]]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    passed: list[dict[str, Any]] = []
    rejected: list[dict[str, Any]] = []
    for event in events:
        window = window_index.get((int(event["dt"]), str(event["code"])))
        features = _anti_long_high_hold_features(window or [])
        enriched = {
            **event,
            "anti_long_high_hold_features": features,
            "anti_long_high_hold_gate": "reject" if features.get("anti_short_high_hold") else "pass",
        }
        if features.get("anti_short_high_hold"):
...
def run_probe(*, db_path: Path, output_root: Path, start_dt: int, end_dt: int, max_codes: int | None = None) -> dict[str, Any]:
    output_dir = output_root / f"{_now_tag()}-a_phase_anti_long_high_hold_oos_v1"
    output_dir.mkdir(parents=True, exist_ok=True)
    con = duckdb.connect(str(db_path), read_only=True)
    try:
        sell_rows = _load_signal_rows(con, start_dt=start_dt, end_dt=end_dt)
        by_code = _load_daily(con, start_dt=start_dt, end_dt=end_dt, history=260, forward=20)
    finally:
        con.close()
    ...
    window_index = _build_window_index(by_code)
    filtered_candidates, rejected_candidates = _apply_anti_long_high_hold(shape_filtered_candidates, window_index)
    selected_by_date = _select_one_per_date(filtered_candidates)
    ...
    coverage = {
        ...
        "anti_long_high_hold_reject_count": len(rejected_candidates),
        "anti_long_high_hold_pass_count": len(filtered_candidates),
    ...
    result = {
        "schema_version": "tradex_short_scene_visual_additive_a_phase_anti_long_high_hold_oos_v1",
        ...
        "fixed_evaluation_conditions": {
            ...
            "changed_axis": "a_phase_100ma_slope_tight_plus_anti_long_high_hold_gate",
            "selection_rule": "previous keep rule plus reject short candidate when close is high in one-year display range and less than 12% below the display-window high",
            "screenshot_feedback_source": "current 4384 screenshot rejected short because visual read was long high-zone tight hold",
        ...
        "observed_branching": {
            ...
            "selection_divergence_reason": "historical_oos_outside_gap_a_phase_candidate_added_only_when_anti_long_high_hold_gate_passes",
        },
        ...
        "remaining_risks": [
            "anti-long-high-hold gate is OHLC proxy informed by one current screenshot rejection",
    ...
    with ledger_path.open("w", encoding="utf-8") as fh:
        for event in [*filtered_candidates, *rejected_candidates]:
