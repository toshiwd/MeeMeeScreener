...
from scripts import tradex_downside_acceleration_selection_v1 as mod
...
def test_downside_acceleration_selector_keep(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    decision = json.loads((out / "downside_acceleration_decision.json").read_text(encoding="utf-8"))
    compare = json.loads((out / "downside_acceleration_compare.json").read_text(encoding="utf-8"))
    no_lookahead = json.loads((out / "no_lookahead_audit.json").read_text(encoding="utf-8"))
    contract = json.loads((out / "downside_acceleration_selection_contract.json").read_text(encoding="utf-8"))
