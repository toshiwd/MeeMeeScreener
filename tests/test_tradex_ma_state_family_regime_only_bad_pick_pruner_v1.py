...
from scripts.tradex_ma_state_family_regime_only_bad_pick_pruner_v1 import (
    REGIME_ONLY_PENALTY,
        ...
        [
            {
                ...
                "family_bad_pick_regime": "C:bullish",
                "dominant_regime_context": "C:bullish",
            }
        ]
    )
    adjusted = _apply_pruner_scores(frame)
    assert adjusted.loc[0, "score_adjustment"] == REGIME_ONLY_PENALTY
    assert adjusted.loc[0, "challenger_score"] == pytest.approx(frame.loc[0, "score"] + REGIME_ONLY_PENALTY)
    assert bool(adjusted.loc[0, "regime_match_penalty_applied"]) is True
    assert adjusted.loc[0, "score_adjustment_reason"] == ["regime_bad_pick_family", "regime_only_match"]


def test_pruner_smoke_run_writes_required_artifacts(tmp_path: Path) -> None:
    output_root = tmp_path / "bad_pick_pruner"
    result = run_ma_state_family_regime_only_bad_pick_pruner_v1(
    ...
    required_files = (
        "run_manifest.json",
        "ma_state_family_regime_only_bad_pick_pruner_v1_compare.json",
        "ma_state_family_regime_only_bad_pick_pruner_v1_decision.json",
        "v1_v1_1_regime_only_delta.json",
    ...
    compare = json.loads((session_dir / "ma_state_family_regime_only_bad_pick_pruner_v1_compare.json").read_text(encoding="utf-8"))
    decision = json.loads((session_dir / "ma_state_family_regime_only_bad_pick_pruner_v1_decision.json").read_text(encoding="utf-8"))
    delta = json.loads((session_dir / "v1_v1_1_regime_only_delta.json").read_text(encoding="utf-8"))
    ...
    assert compare["schema_version"] == "tradex_ma_state_family_regime_only_bad_pick_pruner_v1_compare_v1"
    ...
    assert delta["schema_version"] == "tradex_ma_state_family_regime_only_bad_pick_pruner_v1_delta_v1"
