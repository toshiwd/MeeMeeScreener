...
from scripts import tradex_downside_acceleration_closed_horizon_selection_v2 as mod
...
def test_downside_acceleration_selector_keep(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    decision = json.loads((out / "downside_acceleration_closed_horizon_decision.json").read_text(encoding="utf-8"))
    compare = json.loads((out / "downside_acceleration_closed_horizon_compare.json").read_text(encoding="utf-8"))
    no_lookahead = json.loads((out / "no_lookahead_audit.json").read_text(encoding="utf-8"))
    contract = json.loads((out / "downside_acceleration_closed_horizon_contract.json").read_text(encoding="utf-8"))
...
def test_downside_acceleration_selector_excludes_unknown_horizons(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    no_lookahead = json.loads((out / "no_lookahead_audit.json").read_text(encoding="utf-8"))
    rankings = (out / "downside_acceleration_closed_horizon_monthly_rankings.csv").read_text(encoding="utf-8")
