...
if (-not $ReportPath) {
    $ReportPath = Join-Path $reportDir ("tradex_daily_research_dispatch_{0}.json" -f $stamp)
}
if (-not $TextReportPath) {
    $TextReportPath = Join-Path $reportDir ("tradex_daily_research_dispatch_{0}.txt" -f $stamp)
}
if (-not $LogPath) {
    $LogPath = Join-Path $logDir ("tradex_daily_research_dispatch_{0}.log" -f $stamp)
}
