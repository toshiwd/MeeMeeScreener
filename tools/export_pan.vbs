...
Sub Main()
    ...
    Dim shell, env, localAppData, defaultDataDir, envCode, envOut
    Dim cal, prices
    Dim tsCodes
    ...
    If Len(localAppData) > 0 Then
        defaultDataDir = localAppData & "\MeeMeeScreener\data"
        codesFile = defaultDataDir & "\code.txt"
        outFolder = defaultDataDir & "\txt"
    End If
    envCode = env("PAN_CODE_TXT_PATH")
    If Len(envCode) > 0 Then
        codesFile = envCode
    End If
    envOut = env("PAN_OUT_TXT_DIR")
    If Len(envOut) > 0 Then
        outFolder = envOut
    End If
