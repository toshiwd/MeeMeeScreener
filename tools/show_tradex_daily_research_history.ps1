...
if (-not $ReportPath) {
    $ReportPath = Join-Path $tmpDir ("tradex_daily_research_history_{0}.json" -f $stamp)
}
if (-not $TextReportPath) {
    $TextReportPath = Join-Path $tmpDir ("tradex_daily_research_history_{0}.txt" -f $stamp)
}
if (-not $LogPath) {
    $LogPath = Join-Path $tmpDir ("tradex_daily_research_history_{0}.log" -f $stamp)
}
