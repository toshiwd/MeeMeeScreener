param(
    [string]$PythonExe = "python",
    [string]$AsOfDate = "",
    [string]$PublishId = "",
    [string]$FreshnessState = "fresh",
    [string]$ReportPath = "",
    [string]$TextReportPath = "",
    [string]$LogPath = ""
)

$ErrorActionPreference = "Stop"

$repoRoot = Resolve-Path (Join-Path $PSScriptRoot "..")
$tmpDir = Join-Path $repoRoot "tmp"
if (-not (Test-Path $tmpDir)) {
    New-Item -ItemType Directory -Force -Path $tmpDir | Out-Null
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
if (-not $ReportPath) {
    $ReportPath = Join-Path $tmpDir ("tradex_daily_research_{0}.json" -f $stamp)
}
if (-not $TextReportPath) {
    $TextReportPath = Join-Path $tmpDir ("tradex_daily_research_{0}.txt" -f $stamp)
}
if (-not $LogPath) {
    $LogPath = Join-Path $tmpDir ("tradex_daily_research_{0}.log" -f $stamp)
}

function Write-Log {
    param([string]$Message)
    $line = "{0:u} {1}" -f (Get-Date), $Message
    $line | Tee-Object -FilePath $LogPath -Append | Out-Host
}

try {
    Write-Log "Tradex daily research run started."
    $args = @(
        "-m", "external_analysis",
        "daily-research-run",
        "--freshness-state", $FreshnessState,
        "--report-path", $ReportPath,
        "--text-report-path", $TextReportPath
    )
    if ($AsOfDate) {
        $args += @("--as-of-date", $AsOfDate)
    }
    if ($PublishId) {
        $args += @("--publish-id", $PublishId)
    }
    Write-Log ("Executing: {0} {1}" -f $PythonExe, ($args -join " "))
    $output = & $PythonExe @args 2>&1
    if ($LASTEXITCODE -ne 0) {
        throw "daily research command failed with exit code $LASTEXITCODE"
    }
    foreach ($line in $output) {
        Write-Log ([string]$line)
    }
    Write-Log ("JSON report: {0}" -f $ReportPath)
    Write-Log ("Text report: {0}" -f $TextReportPath)
    Write-Log "Tradex daily research run finished successfully."
} catch {
    Write-Log ("ERROR: " + $_.Exception.Message)
    throw
}
