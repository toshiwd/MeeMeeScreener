...
def get_events(conn, symbols: list[str] | None = None):
    """
    Return trade_events rows in a stable tuple shape even if the underlying table schema differs
    across versions.

    Tuple layout (indexes used by `_map_trade_event`):
    0:id, 1:broker, 2:exec_dt, 3:symbol, 4:action, 5:qty, 6:price, 7:source_row_hash,
    8:created_at, 9:transaction_type, 10:side_type, 11:margin_type
    """
    ...
    for col in select_cols:
        if col and col in columns:
            ...
            # Always expose a 'symbol' slot even if the column is absent.
