...
class TradeParser:
    ...
    def determine_event_kind(label: str, type_label: str = "") -> Tuple[Optional[str], str, str]:
        ...
        if event_kind is None:
            if "信用新規買" in full_label or "買建" in full_label:
                event_kind = "BUY_OPEN"
            elif "信用返済売" in full_label or "売埋" in full_label:
                event_kind = "SELL_CLOSE"
            elif "信用新規売" in full_label or "売建" in full_label:
                event_kind = "SELL_OPEN"
            elif "信用返済買" in full_label or "買埋" in full_label:
                event_kind = "BUY_CLOSE"
            elif "現物買" in full_label or "買付" in full_label:
                event_kind = "BUY_OPEN"
            elif "現物売" in full_label or "売付" in full_label:
                event_kind = "SELL_CLOSE"
    ...
    def parse_rakuten_rows(rows_all: List[List[str]], encoding_used: str = "") -> Dict[str, Any]:
        ...
        get_cell = lambda row, key: TradeParser.normalize_text(row[col_map.get(key, -1)]) if key in col_map else ""
