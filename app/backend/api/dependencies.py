...
def _init_repos(data_dir: str | None, allow_fallback: bool) -> None:
    ...
    with _repo_lock:
        ...
        Path(db_path).expanduser().resolve(strict=False).parent.mkdir(parents=True, exist_ok=True)
        Path(fav_path).expanduser().resolve(strict=False).parent.mkdir(parents=True, exist_ok=True)
